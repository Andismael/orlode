...
function unwrapCompanyId(raw: string | undefined): string {
  if (!raw) return '';
...
router.get('/:companyId', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!snap.exists) {
    ...
    logger.info('[Manifest] No companies/{id} doc, using query overrides', { companyId });
  }

  const company = (snap.data() ?? {}) as {
    name?: string;
    logoUrl?: string;
    pwaLogoUrl?: string; // explicit square logo, preferred over logoUrl when set
    primaryColor?: string;
    accentColor?: string;
  };
  ...
  const nameOverride  = (req.query['name']  as string | undefined)?.trim();
  const logoOverride  = (req.query['logo']  as string | undefined)?.trim();
  const colorOverride = (req.query['color'] as string | undefined)?.trim();
