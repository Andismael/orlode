...
import {
  ...
  getDueReminders,
  markReminderFired,
  listEnabledProfilesForHour,
  lastSessionSummary,
} from '../services/kora/koraMemoryService';
import { koraReply } from '../services/kora/koraChatService';
 ...
 */
router.post('/standalone/checkout', asyncHandler(async (req: Request, res: Response) => {
  const phoneE164 = String(req.body?.phoneE164 ?? '').trim();
  if (!phoneE164) throw new AppError('phoneE164 required', 400);
 ...
 */
router.post('/standalone/webhook/stripe', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (event?.type === 'checkout.session.completed') {
    ...
    if (phoneE164) {
      const { markStandaloneActive } = await import('../services/kora/koraStandaloneService');
 ...
 */
router.post('/cron/checkin', asyncHandler(async (req: Request, res: Response) => {
  if (!checkCronSecret(req)) { res.status(401).json({ success: false }); return; }
  ...
  let dispatched = 0;
  for (let h = 0; h < 24; h++) {
    const profiles = await listEnabledProfilesForHour(h, 500);
    for (const profile of profiles) {
      try {
        ...
        if (Number.isNaN(local) || local !== h) continue;
        ...
        if (profile.phoneE164) {
          try {
            ...
            logger.warn('[Kora] check-in WA send failed', { uid: profile.uid, error: String(sendErr) });
          }
        }
      } catch (perUserErr) {
        logger.warn('[Kora] check-in per-profile failed', { uid: profile.uid, error: String(perUserErr) });
      }
    }
 ...
 */
async function decideCheckInReason(uid: string, companyId: string): Promise<string | null> {
  ...
  try {
    const todays = await db.collection(`companies/${companyId}/koraProfiles/${uid}/reminders`)
      ...
      .limit(1).get();
    if (!todays.empty) {
      const r = todays.docs[0].data() as any;
      return `Rappel programmé aujourd'hui : "${r.title}"`;
    }
  } catch { /* index missing — fall through */ }

  const last = await lastSessionSummary(uid, companyId);
  if (last) {
...
router.post('/cron/reminders', asyncHandler(async (req: Request, res: Response) => {
  ...
  for (const r of reminders) {
    try {
      const profile = await getOrCreateProfile({ uid: r.uid, companyId: r.companyId });
      ...
      if (profile.phoneE164 && (r.channel === 'whatsapp' || r.channel === 'both')) {
        try {
          const { whatsappService } = await import('../services/whatsapp/whatsappService');
          const cfg = await whatsappService.getConfig(profile.companyId).catch(() => null);
          if (cfg) await whatsappService.sendMessage(cfg, profile.phoneE164, reminderMsg);
        } catch (sendErr) {
          logger.warn('[Kora] reminder WA send failed', { reminderId: r.id, error: String(sendErr) });
        }
      }
      await markReminderFired(r.uid, r.companyId, r.id);
      dispatched++;
    } catch (err) {
      logger.warn('[Kora] reminder dispatch failed', { reminderId: r.id, error: String(err) });
    }
  }
...
router.get('/profile', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const uid = req.user?.uid; const companyId = req.user?.companyId;
...
router.patch('/profile', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const uid = req.user?.uid; const companyId = req.user?.companyId;
  if (!uid || !companyId) throw new AppError('Auth required', 401);
  const allowed: Array<keyof import('../services/kora/koraMemoryService').KoraProfile> = [
  ...
  await updateProfile(uid, companyId, patch);
  const profile = await getOrCreateProfile({ uid, companyId });
  res.json({ success: true, data: profile });
}));

router.get('/facts', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const uid = req.user?.uid; const companyId = req.user?.companyId;
  if (!uid || !companyId) throw new AppError('Auth required', 401);
  const facts = await recallFacts({ uid, companyId, limit: 200 });
  res.json({ success: true, data: facts });
}));

router.delete('/facts/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const uid = req.user?.uid; const companyId = req.user?.companyId;
  if (!uid || !companyId) throw new AppError('Auth required', 401);
  const deleted = await forgetFact({ uid, companyId, factIds: [req.params.id] });
 ...
 */
router.post('/activate', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const uid = req.user?.uid; const companyId = req.user?.companyId;
  if (!uid || !companyId) throw new AppError('Auth required', 401);
  const directives = Array.isArray(req.body?.directives)
    ? (req.body.directives as unknown[]).map(d => String(d).trim()).filter(Boolean).slice(0, 5).map(d => d.slice(0, 200))
    : undefined;

  const current = await getOrCreateProfile({ uid, companyId });
  const patch: any = { enabled: true };
  if (directives) patch.directives = directives;
...
router.get('/reminders', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const uid = req.user?.uid; const companyId = req.user?.companyId;
  if (!uid || !companyId) throw new AppError('Auth required', 401);
  const snap = await getFirestore().collection(`companies/${companyId}/koraProfiles/${uid}/reminders`)
    .where('status', '==', 'pending')
    .orderBy('dueAt', 'asc')
    .limit(100).get();
 ...
 */
router.post('/migrate-standalone', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const uid = req.user?.uid; const companyId = req.user?.companyId;
  if (!uid || !companyId) throw new AppError('Auth required', 401);
  const phoneE164 = String(req.body?.phoneE164 ?? '').trim();
  if (!phoneE164) throw new AppError('phoneE164 required', 400);

  const { migrateStandaloneToWorkspace } = await import('../services/kora/koraStandaloneService');
  const result = await migrateStandaloneToWorkspace({ phoneE164, targetCompanyId: companyId, targetUid: uid });
  res.json({ success: true, data: result });
}));

router.delete('/reminders/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const uid = req.user?.uid; const companyId = req.user?.companyId;
  if (!uid || !companyId) throw new AppError('Auth required', 401);
  await getFirestore().doc(`companies/${companyId}/koraProfiles/${uid}/reminders/${req.params.id}`).delete();
