...
import { wemasService } from '../services/wemas/wemasService';
...
router.get('/', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const contracts = await wemasService.listContracts(companyId, { status: status as any, type: type as any });
...
router.get('/stats', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  const stats = await wemasService.getStats(companyId);
  res.json({ success: true, data: stats });
}));

router.get('/portfolios', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  const portfolios = await wemasService.getPortfolios(companyId);
...
router.get('/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  const contract = await wemasService.getContract(companyId, req.params['id']);
...
router.post('/', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!signatoryName || !signatoryEmail) throw new AppError('signatoryName and signatoryEmail required', 400);
  const contract = await wemasService.createContract({ companyId, createdBy: req.user?.uid, ...req.body });
...
router.patch('/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  await wemasService.updateContract(companyId, req.params['id'], req.body);
...
router.post('/:id/send', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  const uniqueLink = await wemasService.sendForSignature(companyId, req.params['id']);
...
router.delete('/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  await wemasService.deleteContract(companyId, req.params['id']);
  res.json({ success: true });
}));

// ── AI Contract Generation ───────────────────────────────────────────────────

const CONTRACT_TEMPLATES: Record<string, (name: string, org: string) => string> = {
...
router.post('/generate', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!signatoryName) throw new AppError('signatoryName required', 400);
...
router.get('/comments/:contractId', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  res.json({ success: true, data: await wemasService.listComments(companyId, req.params['contractId']) });
}));

router.post('/comments', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  const result = await wemasService.addComment(companyId, {
    ...req.body,
...
router.delete('/comments/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  await wemasService.deleteComment(companyId, req.params['id']);
  res.json({ success: true });
}));
...
router.get('/templates/list', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  res.json({ success: true, data: await wemasService.listTemplates(companyId) });
}));

router.post('/templates', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  const template = await wemasService.createTemplate(companyId, req.body);
  res.status(201).json({ success: true, data: template });
}));

router.patch('/templates/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  await wemasService.updateTemplate(companyId, req.params['id'], req.body);
  res.json({ success: true });
}));
...
router.delete('/templates/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  await wemasService.deleteTemplate(companyId, req.params['id']);
  res.json({ success: true });
}));

// ── Portfolio Documents ──────────────────────────────────────────────────────

router.get('/documents/:email', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  res.json({ success: true, data: await wemasService.listDocuments(companyId, req.params['email']) });
}));

router.post('/documents', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  const doc = await wemasService.addDocument(companyId, { ...req.body, uploadedBy: req.user?.uid });
  res.status(201).json({ success: true, data: doc });
}));

router.delete('/documents/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  await wemasService.deleteDocument(companyId, req.params['id']);
  res.json({ success: true });
}));

// ── Portfolio Notes ──────────────────────────────────────────────────────────

router.get('/notes/:email', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  res.json({ success: true, data: await wemasService.listNotes(companyId, req.params['email']) });
}));

router.post('/notes', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  const note = await wemasService.addNote(companyId, { ...req.body, createdBy: req.user?.uid });
  res.status(201).json({ success: true, data: note });
}));

router.patch('/notes/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  await wemasService.updateNote(companyId, req.params['id'], req.body.content);
  res.json({ success: true });
}));

router.delete('/notes/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  await wemasService.deleteNote(companyId, req.params['id']);
  res.json({ success: true });
}));

// ── Upload Requests ──────────────────────────────────────────────────────────

router.post('/upload-requests', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  const request = await wemasService.createUploadRequest(companyId, { ...req.body, createdBy: req.user?.uid });
  res.status(201).json({ success: true, data: request });
}));

router.delete('/upload-requests/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const companyId = req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  await wemasService.deleteUploadRequest(companyId, req.params['id']);
  res.json({ success: true });
}));
...
publicContractRouter.get('/contract/:uniqueLink', asyncHandler(async (req: Request, res: Response) => {
  const contract = await wemasService.getContractByUniqueLink(req.params['uniqueLink']);
  if (!contract) throw new AppError('Contrat introuvable', 404);
  res.json({ success: true, data: contract });
}));

publicContractRouter.post('/contract/:uniqueLink/sign', asyncHandler(async (req: Request, res: Response) => {
  const signingLink = req.params['uniqueLink'];
  const contract = await wemasService.getContractByUniqueLink(signingLink);
  if (!contract) throw new AppError('Contrat introuvable', 404);
  const { signatureData } = req.body;
  if (!signatureData) throw new AppError('signatureData is required', 400);
  const result = await wemasService.signContract(
    contract.companyId, contract.id, signatureData,
...
publicContractRouter.get('/upload/:token', asyncHandler(async (req: Request, res: Response) => {
  const request = await wemasService.getUploadRequestByToken(req.params['token']);
  if (!request) throw new AppError('Lien introuvable', 404);
  if (request.expiresAt && new Date(request.expiresAt) < new Date()) throw new AppError('Lien expire', 410);
  res.json({ success: true, data: request });
}));

// GET /api/public/host-notifications/:id/status — kiosk polls this to see if the host replied
publicContractRouter.get('/host-notifications/:id/status', asyncHandler(async (req: Request, res: Response) => {
  const id = req.params['id'];
  if (!id) throw new AppError('ID requis', 400);
  ...
  if (!snap || snap.empty) {
    ...
    if (!match) { res.status(404).json({ success: false, message: 'not found' }); return; }
    const data = match.data();
    res.json({
      success: true,
      status:        (data['status'] as string) ?? 'pending',
      replyAction:   (data['replyAction'] as string) ?? null,
      repliedAt:     data['repliedAt'] ?? null,
      hostName:      (data['hostName'] as string) ?? null,
      hostPresent:   (data['hostPresent'] as boolean) ?? false,
    });
    return;
  }
...
publicContractRouter.get('/host-reply/:token', asyncHandler(async (req: Request, res: Response) => {
  ...
  const hostName = (data['hostName'] as string) ?? 'L\'hôte';
  const visitorName = (data['visitorName'] as string) ?? 'Le visiteur';
...
publicContractRouter.post('/upload-document', asyncHandler(async (req: Request, res: Response) => {
  const body = req.body as Record<string, unknown>;
  const token = body['token'] as string;
  if (!token) throw new AppError('Token requis', 400);
  const request = await wemasService.getUploadRequestByToken(token);
  if (!request) throw new AppError('Lien introuvable', 404);
  if (request.expiresAt && new Date(request.expiresAt) < new Date()) throw new AppError('Lien expire', 410);

  const doc = await wemasService.addDocument(request.companyId, {
    signatoryEmail: request.signatoryEmail,
    signatoryName: request.signatoryName,
    documentType: (body['documentType'] as string) ?? 'autre',
    label: (body['label'] as string) ?? 'Document',
