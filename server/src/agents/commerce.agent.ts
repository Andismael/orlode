...
import { syncProductToMeta, deleteProductFromMeta } from '../services/commerce/metaCatalogSync';
import { isStoreOpen, isStoreOpenAt } from '../utils/openingHoursCheck';
import { findExternalBlockConflict } from '../services/commerce/icalImport';
...
export interface DeliveryZone {
  name: string;
  fee: number;
  freeAbove?: number;       // free delivery if order total ≥ freeAbove
}

export interface Store {
  name: string;
  slug?: string;            // public URL: /shop/{slug}. Globally unique.
  ownerPhone: string;       // E.164
  currency: string;         // 'XOF' default Phase 1
  country: string;          // 'CI' default Phase 1
  status: 'active' | 'suspended' | 'trial';
  paymentInstructions?: string; // free-text owner-defined: "Wave +225 07XX, ou paiement à la livraison"
  deliveryZones?: DeliveryZone[];
  // Storefront customization (public page)
  logoUrl?: string;
  coverImageUrl?: string;
  accentColor?: string;     // hex like '#10B981'. Default emerald.
  // Loyalty program (Phase 3)
  loyaltyEnabled?: boolean;     // default true
  loyaltyPointsPerUnit?: number;// 1 point per N currency. Default 100 (= 1pt per 100 XOF)
  loyaltyThreshold?: number;    // points needed to redeem. Default 100
  loyaltyDiscountPct?: number;  // % off. Default 10
  // PIN — set by owner in Settings, asked for ultra-sensitive actions
  // (export customers, refund, delete all). 4-6 digits, stored hashed.
  pinHash?: string;
  /** Free-text opening hours, displayed as-is on the public page.
   * Examples: "Lun-Ven 12h-22h, Sam-Dim 18h-23h", "Tous les jours 18h-2h", "Fermé le lundi". */
  openingHours?: string;
  /** Restaurant sub-type for sub-pack tweaks (delivery off for bar, etc.) */
  establishmentType?: 'restaurant' | 'maquis' | 'bar';
  /** Salon sub-type — purely cosmetic (label + emoji), all features identical. */
  salonType?: 'coiffure' | 'esthetique' | 'spa' | 'barber';
  /** Free-text address ("Cocody, Riviera 3, près de la pharmacie centrale"). */
  address?: string;
  /** Google Maps link the owner pastes — also accepts maps.app.goo.gl, etc. */
  googleMapsUrl?: string;
  /** Optional GPS coords — when present we'll send a native WhatsApp location ping. */
  latitude?: number;
  longitude?: number;
  /** Pack type. 'residence' = Booking/Airbnb-style short-term rental (each store = 1 unit). */
  businessType?: 'boutique' | 'restaurant' | 'hotel' | 'service' | 'health' | 'realestate' | 'residence';
  // ── Airbnb-style fields (used when businessType = 'residence') ──
  /** "entire" | "private_room" | "shared_room" — what the guest gets. */
  ...
  /** Max guests allowed. */
  maxGuests?: number;
  /** Free children threshold (default 12). */
  childrenFreeUnder?: number;
  /** Allow extra guests above capacity (paid surcharge). */
  allowExtraGuests?: boolean;
  /** Max extra guests when allowed. */
  maxExtraGuests?: number;
  city?: string;
  neighborhood?: string;
  /** Price per night when businessType=residence. */
  pricePerNight?: number;
  /** Optional cleaning fee per stay. */
  cleaningFee?: number;
  /** Amenities list (wifi, ac, kitchen, parking, pool, ...). */
  amenities?: string[];
  /** External booking URL — when set, "Réserver" redirects there. */
  affiliateUrl?: string;
  /** When true, customer's booking is auto-confirmed (no manual approval). */
  instantBooking?: boolean;
  /** Show contact form instead of booking flow. */
  useContactForm?: boolean;
  /** Featured listing — surfaced first if multiple residences. */
  featured?: boolean;
  /** Description for the listing (for residence pack only — distinct from
   *  paymentInstructions which is the merchant payment note). */
  longDescription?: string;
  /** Salon/health staff list. Each entry is a bookable practitioner. The
   *  bot's `bookAppointment` tool already accepts `practitionerName`; this
   *  list lets the public page (and clone) propose a choice. */
...
export interface Practitioner {
  /** Stable id (slug-friendly). */
  id: string;
  /** Display name shown to clients. */
  name: string;
  /** Optional role label ("Coiffeuse", "Manucure", "Médecin généraliste"). */
  role?: string;
  /** Optional photo URL. */
  photoUrl?: string;
  /** Free-text working hours override — falls back to store.openingHours. */
  workingHours?: string;
  /** Toggle off temporarily without deleting. */
  active?: boolean;
}
...
export async function setStorePin(
  companyId: string, storeId: string, pin: string,
): Promise<{ ok: boolean; reason?: string }> {
  if (!/^\d{4,6}$/.test(pin)) return { ok: false, reason: 'PIN doit faire 4 à 6 chiffres.' };
  await getFirestore().doc(`companies/${companyId}/stores/${storeId}`)
...
export async function verifyStorePin(
  ...
  const stored = (snap.data() as Store | undefined)?.pinHash;
  return !!stored && stored === hashPin(pin);
}

export async function markPinVerifiedInSession(
  companyId: string, storeId: string, ownerPhone: string,
): Promise<void> {
  await getFirestore()
...
export async function isPinVerifiedInSession(
  companyId: string, storeId: string, ownerPhone: string,
): Promise<boolean> {
  ...
  const d = snap.data() as { pinExpiresAt?: { toDate: () => Date } | Date };
...
export function isUltraSensitive(text: string): boolean {
  if (!text) return false;
  const t = text.toLowerCase();
  return (
    /\b(export\w*|exporte\w*)\b.*\b(client|commande|tout|tous|all)/i.test(t) ||
...
export async function findStoreBySlug(
  slug: string,
): Promise<{ companyId: string; storeId: string; store: Store } | null> {
  if (!slug) return null;
  const snap = await getFirestore().collectionGroup('stores')
    .where('slug', '==', slug).limit(1).get().catch(() => null);
  if (!snap || snap.empty) return null;
  const doc = snap.docs[0];
  const parent = doc.ref.parent.parent; // companies/{companyId}
  if (!parent) return null;
  return { companyId: parent.id, storeId: doc.id, store: doc.data() as Store };
}
 ...
 */
export async function ensureStoreSlug(
  companyId: string, storeId: string, store: Store,
): Promise<string> {
  if (store.slug) return store.slug;
  const slug = await generateUniqueSlug(store.name);
  await getFirestore().doc(`companies/${companyId}/stores/${storeId}`)
    .update({ slug, updatedAt: new Date() });
  logger.info('[Commerce] Backfilled store slug', { companyId, storeId, slug });
  return slug;
}
...
export interface Product {
  ...
  currency: string;
  description?: string;
  /** Primary image (legacy single-image field). For multi-image products, equals imageUrls[0]. */
  imageUrl?: string;
  /** Gallery — set when the owner sends multiple photos for the same article. */
  imageUrls?: string[];
  /** Optional short video (≤30s) — used by realestate/hotel for tours. */
  videoUrl?: string;
  /** Featured = "plat du jour" / produit phare — surfaced first on the public page with a badge. */
  featured?: boolean;
  // ── Airbnb-style listing fields (real estate / hotel rooms) ──
  /** Listing category (apartment, house, villa, studio, room) — Airbnb-like. */
  listingCategory?: string;
  /** Room type for shared listings: 'entire' | 'private_room' | 'shared_room'. */
  roomType?: 'entire' | 'private_room' | 'shared_room';
  /** Max guests allowed (capacity). */
  maxGuests?: number;
  /** Free children threshold — kids under N years are not charged. Default 12. */
  childrenFreeUnder?: number;
  /** Allow extra guests above capacity ? (paid surcharge usually). */
  allowExtraGuests?: boolean;
  /** Max extra guests above capacity (when allowExtraGuests is true). */
  maxExtraGuests?: number;
  // ── Localisation ──
  city?: string;
  neighborhood?: string;
  ...
  // ── Booking behavior ──
  /** External affiliate URL — when set, booking redirects there instead. */
  affiliateUrl?: string;
  /** Show a contact form instead of the regular booking flow. */
  useContactForm?: boolean;
  /** When true, no manual approval needed — bookings auto-confirmed. */
  instantBooking?: boolean;
  /** Amenities list (wifi, ac, kitchen, parking, pool, ...). */
  amenities?: string[];
  /** Before/after gallery — owner uploads photos clients see in galleries.
   *  Each entry can also flag explicit *signed* consent for the photo to be
   *  used as an AI style reference (faces are biometric data — usage as IA
   *  reference requires written consent under GDPR / loi 2013-450 CI etc).
   *  Without consentForAI:true, a photo is *displayed* but never sent to the
   *  IA face-preview pipeline as a reference. */
  examplePhotos?: Array<{ url: string; consentForAI?: boolean; label?: string }> | string[];
  stockQty: number;
  status: 'draft' | 'active' | 'out_of_stock' | 'archived';
  createdBy: 'whatsapp' | 'admin' | 'ai';
  createdAt: Date;
  updatedAt: Date;
}
...
const reservationMinutes = (t: string): number => {
...
export async function findReservationConflict(args: {
  ...
  slotMinutes: number;
  scope:
    | { kind: 'service'; serviceId: string; practitionerName?: string }
    | { kind: 'health'; practitionerName?: string }
  ...
  let q = db.collection(`companies/${args.companyId}/stores/${args.storeId}/reservations`)
    .where('date', '==', args.date) as FirebaseFirestore.Query;
  if (args.scope.kind === 'service') q = q.where('serviceId', '==', args.scope.serviceId);
  if (args.scope.kind === 'table')   q = q.where('tableId', '==', args.scope.tableId);
  ...
  for (const d of snap.docs) {
    ...
    if (Math.abs(reservationMinutes(r.time) - newStart) >= args.slotMinutes) continue;
    ...
    if (askedPract && r.practitionerName && askedPract !== r.practitionerName) continue;
    return { time: r.time };
  }
  return null;
}
...
// One subcollection `tables` per restaurant store. A reservation can pin a
// specific table (`tableId`) — the booking endpoint then enforces a single
// reservation per table per 2-hour slot (anti-double-booking).
export interface Table {
  number: string;                // "T1", "Salle 4"
  capacity: number;              // max party size
  zone?: 'indoor' | 'terrace' | 'private' | string;
...
export interface Order {
  ...
  total: number;
  currency: string;
  paymentMethod: 'cod' | 'mobile_money_manual';
...
export function parseProductCaption(caption: string): {
  name: string; price: number; stock?: number; currency?: string;
} | null {
  ...
  if (currencyMatch) {
    ...
    if (sym === '$') detectedCurrency = 'USD';
    else if (sym === '€') detectedCurrency = 'EUR';
    else if (sym === '£') detectedCurrency = 'GBP';
    else if (sym === '¥') detectedCurrency = 'JPY';
    else if (sym === 'fcfa' || sym === 'cfa') detectedCurrency = 'XOF';
    else detectedCurrency = sym.toUpperCase();
  }
 ...
 */
export async function findStoreByOwnerPhone(
  fromPhone: string,
  ...
  if (scopedCompanyId) {
    for (const v of variants) {
      const snap = await db.collection(`companies/${scopedCompanyId}/stores`)
        ...
        .get()
        .catch((err) => {
          logger.warn('[Commerce] scoped store query failed', { companyId: scopedCompanyId, variant: v, error: String(err) });
          return null;
        });
      if (snap && !snap.empty) {
        const doc = snap.docs[0];
        logger.info('[Commerce] Owner store matched (scoped)', {
    ...
    if (allSnap && !allSnap.empty) {
      for (const doc of allSnap.docs) {
        ...
        if (storedDigits && storedDigits === digits) {
          logger.info('[Commerce] Owner store matched (digit-compare fallback)', {
      ...
      logger.info('[Commerce] No owner-phone match within company', {
        from: fromPhone,
        digits,
        companyId: scopedCompanyId,
        storesChecked: allSnap.docs.map(d => ({
          id: d.id,
          storedOwnerPhone: (d.data() as Store).ownerPhone,
        })),
      });
    }
    return null;
  ...
  for (const v of variants) {
    const snap = await db.collectionGroup('stores')
      ...
      .get()
      .catch((err) => {
        logger.warn('[Commerce] collectionGroup query failed', { variant: v, error: String(err) });
        return null;
      });
    if (snap && !snap.empty) {
      const doc = snap.docs[0];
  ...
  logger.info('[Commerce] No store matched owner phone (collectionGroup)', { from: fromPhone, variantsTried: variants });
  return null;
}
...
export async function startOwnerOtp(
  companyId: string, storeId: string, phone: string,
): Promise<string> {
...
export async function verifyOwnerOtp(
  companyId: string, storeId: string, phone: string, code: string,
): Promise<{ ok: boolean; reason?: string }> {
  const db = getFirestore();
  const ref = db.collection(`companies/${companyId}/stores/${storeId}/sessions`)
    .doc(normalizePhone(phone));
  const snap = await ref.get();
  if (!snap.exists) return { ok: false, reason: 'no_session' };
  const data = snap.data() as { otpHash?: string; otpExpiresAt?: { toDate: () => Date } | Date; attempts?: number };
  ...
  if ((data.attempts ?? 0) >= 5) return { ok: false, reason: 'too_many_attempts' };
...
export async function isOwnerSessionValid(
  companyId: string, storeId: string, phone: string,
): Promise<boolean> {
  const snap = await getFirestore()
    .collection(`companies/${companyId}/stores/${storeId}/sessions`)
    ...
    .get();
  if (!snap.exists) return false;
  const d = snap.data() as { verified?: boolean; expiresAt?: { toDate: () => Date } | Date };
...
async function downloadMediaFromMeta(
  ...
  if (!mediaRes.ok) throw new Error(`Meta media URL failed: ${mediaRes.status}`);
  const mediaData = await mediaRes.json() as { url?: string; mime_type?: string };
  if (!mediaData.url) throw new Error('No media URL from Meta');
...
async function uploadBufferToStorage(
  buffer: Buffer, mimeType: string, companyId: string, storeId: string,
): Promise<string> {
  const ext = mimeType.split('/')[1]?.split('+')[0] ?? 'jpg';
  const fileName = `companies/${companyId}/stores/${storeId}/products/${randomBytes(8).toString('hex')}.${ext}`;
...
// ── Gemini Vision analysis: enrich product from photo ────────────────────────
export interface ProductAnalysis {
  category: string;        // ex: "Vêtements traditionnels"
  description: string;     // marketing pitch 1-2 sentences
  tags: string[];          // ex: ["chemise", "africain", "homme", "wax"]
  colors: string[];        // ex: ["bleu", "rouge", "jaune"]
  confidence: 'high' | 'medium' | 'low';
}

// ── Rich product draft — used by the dashboard "✨ Catalogue IA" flow.
// Takes minimal inputs (photo + optional text/price/url) and returns a
// complete editable draft. Distinct from analyzeProductImage which serves
// the WhatsApp photo-upload path with simpler output.
export interface ProductDraft {
  name: string;
  description: string;          // 1-2 sentences, vendeuse
  descriptionLong: string;      // storytelling 4-6 sentences for Quick View
  category: string;             // PARENT category (Vêtements, Chaussures, Cosmétique...)
  subcategory?: string;         // specific type (Polo, Sneakers, Crème...) — affiché en chip sur la card
  brand?: string;               // detected if visible (Tommy, Lacoste, etc.)
  tags: string[];
  colors: string[];
  /** Tailles/variantes proposées si pertinent (S, M, L pour vêtement, 39-44 pour chaussure, 256GB pour téléphone…) */
  variants: Array<{ name: string; size?: string; color?: string }>;
  suggestedPrice?: number;      // if owner gave none, suggest one
  suggestedOriginalPrice?: number; // for crossed-out promo effect
  photoQualityScore: number;    // 1-5
  photoSuggestion?: string;     // e.g. "Fond carrelage — je peux retoucher en crème uniforme"
  suggestedBadges: Array<'featured' | 'rare' | 'promo' | 'new'>;
  confidence: 'high' | 'medium' | 'low';
}
...
async function fetchProductPageText(url: string): Promise<string | null> {
  try {
    ...
    logger.warn('[Commerce] fetchProductPageText failed', { url, error: String(err) });
    return null;
  }
}

export async function generateProductDraft(args: {
  imageBuffer?: Buffer;
  mimeType?: string;
  freeText?: string;
  productUrl?: string;
...
- JSON pur, rien d'autre.` };
    ...
    const safeMime = args.imageBuffer && args.mimeType
      ? ((args.mimeType.startsWith('image/') ? args.mimeType : 'image/jpeg') as 'image/jpeg' | 'image/png' | 'image/webp')
      : null;
    ...
    const parsed = JSON.parse(cleaned) as Partial<ProductDraft> & { variants?: Array<{ name?: string; size?: string; color?: string }> };
    ...
    return {
      ...
      variants: Array.isArray(parsed.variants)
        ? (parsed.variants as Array<{ name?: string; size?: string; color?: string }>)
    ...
    logger.error('[Commerce] generateProductDraft failed', { error: err instanceof Error ? err.message : String(err) });
    return null;
  }
}
...
export async function analyzeProductImage(
  ...
  try {
    ...
    const safeMime = (mimeType.startsWith('image/') ? mimeType : 'image/jpeg') as 'image/jpeg' | 'image/png' | 'image/webp';
...
- Couleurs : noms simples ("bleu", "doré"), pas de "bleu-marine 70%".`,
    ...
    if (!parsed.category || !parsed.description) return null;
    return {
      category: parsed.category,
      description: parsed.description,
      tags: Array.isArray(parsed.tags) ? parsed.tags.slice(0, 8) : [],
    ...
    logger.warn('[Commerce] Vision analysis failed (non-blocking)', { error: err instanceof Error ? err.message : err });
    return null;
  }
}

// ── Variant parser: handles multiple patterns ──────────────────────────────
//   • "tailles S M L XL"             → sizes, default stock 1 per size
//   • "couleurs rouge bleu"          → colors, default stock 1 per color
//   • "S 2 M 1 XL 7"                 → sizes with explicit stocks
//   • "S 2. M 1. XL 7"               → sizes with stocks (dots/periods OK)
//   • "S=2 M=1 L=5"                  → sizes with stocks (= notation)
//   • "rouge 5 bleu 3 vert 2"        → colors with stocks
export interface ParsedVariants {
  type: 'sizes' | 'colors' | 'mixed';
  values: string[];                     // ex: ["S", "M", "L", "XL"]
  stocks?: number[];                    // optional, parallel to values
}

const KNOWN_SIZES = ['XS', 'S', 'M', 'L', 'XL', 'XXL', 'XXXL', '2XL', '3XL', '4XL'];

export function parseVariantReply(text: string): ParsedVariants | null {
  if (!text) return null;
  ...
  if (sizesMatch?.[2]) {
    ...
    if (tokens.length > 0) return { type: 'sizes', values: tokens };
  }
  ...
  if (colorsMatch?.[2]) {
    ...
    if (tokens.length > 0) return { type: 'colors', values: tokens };
  }
...
export async function applyVariantsToLastProduct(
  companyId: string, storeId: string, ownerPhone: string, parsed: ParsedVariants,
): Promise<{ ok: boolean; productName?: string; variantCount?: number }> {
  const db = getFirestore();
  const sessionRef = db.collection(`companies/${companyId}/stores/${storeId}/sessions`)
    .doc(normalizePhone(ownerPhone));
  const sessionSnap = await sessionRef.get();
  const lastProductId = (sessionSnap.data() as { lastProductId?: string } | undefined)?.lastProductId;
  if (!lastProductId) return { ok: false };

  const productRef = db.doc(`companies/${companyId}/stores/${storeId}/products/${lastProductId}`);
  const productSnap = await productRef.get();
  if (!productSnap.exists) return { ok: false };
  ...
  const totalStock = variants.reduce((sum, v) => sum + ((v.stockQty as number) ?? 0), 0);
...
// ── Helper: handle owner photo upload (the WAOUH path) ───────────────────────
export interface PhotoUploadResult {
  productId: string;
  reply: string;     // ready-to-send WhatsApp text
}

export async function handleOwnerPhotoUpload(args: {
  companyId: string;
  storeId: string;
  ownerPhone: string;
  imageId: string;
  ...
  const sessionData = sessionSnap?.data() as { awaitingMorePhotosFor?: string; lastProductId?: string } | undefined;
    ...
    try {
      ...
      if (!productSnap.exists) {
        ...
        const newImageUrl = await uploadBufferToStorage(buffer, mimeType, companyId, storeId)
          .catch(err => { logger.error('[Commerce] Storage upload failed (multi)', { error: String(err) }); return undefined; });
        if (!newImageUrl) {
        ...
        logger.info('[Commerce] Photo appended to product', {
      ...
      logger.error('[Commerce] Multi-photo append failed', { error: err instanceof Error ? err.message : err });
  ...
  const storeCurrency = (storeSnapForCtx.data() as Store | undefined)?.currency ?? 'XOF';
  ...
  try {
    ...
    const [uploadedUrl, generatedDraft] = await Promise.all([
      uploadBufferToStorage(buffer, mimeType, companyId, storeId).catch(err => {
        logger.error('[Commerce] Storage upload failed', { error: String(err) });
        return undefined;
      }),
    ...
    logger.error('[Commerce] Image download/process failed', { error: String(err), imageId });
  ...
  const product: Product & {
    category?: string; tags?: string[]; colors?: string[];
    brand?: string; descriptionLong?: string; originalPrice?: number;
    featured?: boolean; aiAnalysis?: ProductDraft | null;
  } = {
    name: parsed.name,
    price: parsed.price,
    currency: productCurrency,
  ...
  void syncProductToMeta(companyId, storeId, productRef.id, {
    name: product.name,
    price: product.price,
  ...
  logger.info('[Commerce] Product created from photo', {
  ...
  if (productStock !== 1) reply += ` · 📦 Stock : ${productStock}`;
  reply += '\n';
  if (draft && draft.confidence !== 'low') {
    if (draft.brand) reply += `🏷 *Marque* : ${draft.brand}\n`;
    reply += `📂 *Catégorie* : ${draft.category}\n`;
    if (draft.description) reply += `📝 ${draft.description}\n`;
    const badgeLabels = draft.suggestedBadges.map(b =>
      b === 'featured' ? '⭐ Coup de cœur' :
      b === 'rare' ? '💎 Rare' :
      b === 'promo' ? '🏷 Promo' : '✨ Nouveau'
    );
    if (badgeLabels.length > 0) reply += `\n🎯 *Badges* : ${badgeLabels.join(' · ')}\n`;
...
// ── Telegram equivalent of handleOwnerPhotoUpload ──────────────────────────
// Same Agent Catalogue flow but the buffer comes from telegram CDN and the
// owner is keyed by telegramId (not phone). Stores `ownerTelegramId` on the
// store for first-claim binding, then enforces it on subsequent uploads.
export interface TelegramPhotoUploadArgs {
  companyId: string;
  telegramId: string;        // Telegram user id of the sender
  telegramName?: string;     // first_name / username for nicer replies
  imageBuffer: Buffer;
  mimeType: string;
  caption?: string;
}

export async function handleOwnerTelegramPhoto(args: TelegramPhotoUploadArgs): Promise<{ productId: string; reply: string }> {
  ...
  const ownedAll = stores.filter(d => (d.data() as { ownerTelegramId?: string }).ownerTelegramId === telegramId);
  ...
  if (ownedAll.length > 1 && caption) {
    ...
    if (intent) {
      const match = ownedAll.find(d => (d.data() as { businessType?: string }).businessType === intent);
      if (match) owned = match;
    }
  }
  const claimable = stores.find(d => !(d.data() as { ownerTelegramId?: string }).ownerTelegramId);
  let storeDoc = owned;
  if (!storeDoc && claimable) {
    ...
    const claimableData = claimable.data() as { ownerPhone?: string };
    ...
    storeDoc = claimable;
    logger.info('[Telegram] Owner bound to store (no-phone bootstrap)', { companyId, storeId: claimable.id, telegramId });
  }
  if (!storeDoc) {
  ...
  const sessionSnap = await sessionRef.get().catch(() => null);
  const sessionData = sessionSnap?.data() as { awaitingMorePhotosFor?: string } | undefined;
  const awaitingFor = sessionData?.awaitingMorePhotosFor;
  const isAppending = !!awaitingFor && !parsed;

  if (isAppending && awaitingFor) {
    try {
      const bt = (store as Store & { businessType?: string }).businessType ?? 'boutique';
      ...
      if (targetSnap.exists) {
        ...
        if (newUrl) {
          const data = targetSnap.data() as { name?: string; imageUrl?: string; imageUrls?: string[]; coverImageUrl?: string };
          const existing = (data.imageUrls && data.imageUrls.length > 0)
            ? data.imageUrls
      ...
      logger.error('[Telegram] Multi-photo append failed', { error: String(err) });
    }
  }
  ...
  const [imageUrl, draft] = await Promise.all([
    uploadBufferToStorage(imageBuffer, mimeType, companyId, storeId).catch(err => {
      logger.error('[Telegram] Storage upload failed', { error: String(err) });
  ...
  //                    salon / santé / immo)
  const businessType = (store as Store & { businessType?: string }).businessType ?? 'boutique';
  let createdId = '';
  ...
  if (businessType === 'hotel') {
    ...
    const storeExt = store as Store & { imageUrls?: string[] };
  ...
  if (draft && draft.confidence !== 'low') {
    ...
    reply += `📂 *Catégorie* : ${draft.category}\n`;
    if (draft.description) reply += `📝 ${draft.description}\n`;
    const labels = draft.suggestedBadges.map(b =>
      b === 'featured' ? '⭐ Coup de cœur' : b === 'rare' ? '💎 Rare' : b === 'promo' ? '🏷 Promo' : '✨ Nouveau'
    );
    if (labels.length > 0) reply += `\n🎯 *Badges* : ${labels.join(' · ')}\n`;
    if (draft.suggestedOriginalPrice && draft.suggestedOriginalPrice > finalPrice && businessType !== 'hotel' && businessType !== 'residence') {
      reply += `💸 Prix barré suggéré : *${draft.suggestedOriginalPrice.toLocaleString('fr-FR')} ${productCurrency}* (effet promo)\n`;
    }
  }
  if (draft?.photoSuggestion) reply += `\n⚠️ *Suggestion photo* : ${draft.photoSuggestion}\n`;
  const verticalLabel =
    businessType === 'hotel' ? 'ton hôtel' :
...
export const listProductsTool = ai.defineTool(
  {
    ...
    inputSchema: StoreContext.extend({
      query: z.string().optional().describe('Optional text filter on product name'),
    }),
    outputSchema: z.object({
      products: z.array(z.object({
        id: z.string(), name: z.string(), price: z.number(), currency: z.string(),
        imageUrl: z.string().optional(), stockQty: z.number(), description: z.string().optional(),
      })),
      shopLink: z.string().describe('Public storefront URL the customer can open to browse products with images and click "Commander". ALWAYS include this in your reply.'),
    }),
  },
  async ({ companyId, storeId, query }) => {
    ...
    const storeData = storeSnap.data() as Store | undefined;
...
export function matchDeliveryZone(
  addressOrZone: string | undefined,
  zones: DeliveryZone[] | undefined,
): DeliveryZone | null {
  if (!addressOrZone || !zones || zones.length === 0) return null;
...
export const placeOrderTool = ai.defineTool(
  {
    ...
    inputSchema: StoreContext.extend({
      ...
      paymentMethod: z.enum(['cod', 'mobile_money_manual']).default('cod'),
      deliveryAddress: z.string().optional().describe('Customer-provided address. Used to match a delivery zone for fee calculation.'),
      deliveryZone: z.string().optional().describe('Optional zone hint extracted by the agent (e.g. "Cocody"). Falls back to scanning deliveryAddress.'),
    }),
    outputSchema: z.object({
      success: z.boolean(),
      ...
      zoneMatched: z.string().optional(),
      error: z.string().optional(),
      confirmationMessage: z.string().optional().describe('Pre-formatted message to send back to customer — agent should send it verbatim.'),
    }),
  },
  async ({ companyId, storeId, customerName, customerPhone, items, paymentMethod, deliveryAddress, deliveryZone }) => {
    ...
    const hoursCheck = isStoreOpen({ openingHours: store.openingHours });
    ...
    const productSnaps = await Promise.all(
      items.map(it => db.doc(`companies/${companyId}/stores/${storeId}/products/${it.productId}`).get()),
    );
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (!snap.exists) { stockProblems.push(`Produit introuvable (${it.productId})`); continue; }
      const p = snap.data() as Product;
      if (p.status !== 'active') { stockProblems.push(`${p.name} — non disponible`); continue; }
    ...
    const matchedZone =
      matchDeliveryZone(deliveryZone, store.deliveryZones) ??
    ...
    if (loyaltyEnabled && phoneKey) {
      try {
        ...
        if (customerSnap.exists) {
          loyaltyPointsBefore = ((customerSnap.data() as { loyaltyPoints?: number } | undefined)?.loyaltyPoints) ?? 0;
    ...
    const orderNumber = `ORL-${datePart}-${randomBytes(2).toString('hex').toUpperCase()}`;
    ...
    await db.runTransaction(async tx => {
      ...
      for (let i = 0; i < items.length; i++) {
        if (!txSnaps[i].exists) continue;
        const p = txSnaps[i].data() as Product;
        const remaining = (p.stockQty ?? 0) - items[i].qty;
        if (remaining < 0) throw new Error(`Stock épuisé pour ${p.name} pendant la transaction.`);
        tx.update(refs[i], {
      ...
      const order: Order & {
        deliveryAddress?: string; deliveryZone?: string;
        subtotal?: number; deliveryFee?: number;
        loyaltyDiscount?: number; loyaltyPointsRedeemed?: number;
      } = {
        orderNumber,
        customerName,
        customerPhone,
    ...
    logger.info('[Commerce] Order created', {
    ...
    const confirmationMessage =
      `✅ *Commande confirmée* — n°${orderNumber}\n\n` +
      `${itemsBlock}\n\n` +
    ...
    }).catch(err => logger.warn('[Commerce] Owner notification failed (non-blocking)', { error: String(err) }));

    // 6b. Stock-low alert per item (only for products that just dropped <=2)
    notifyOwnerOfLowStock({
      companyId, storeId, ownerPhone: store.ownerPhone,
      productSnaps, items, currency,
    }).catch(err => logger.warn('[Commerce] Low stock alert failed (non-blocking)', { error: String(err) }));
    ...
    }).catch(err => logger.warn('[Commerce] Lead bridging failed (non-blocking)', { error: String(err) }));
    ...
    }).catch(err => logger.warn('[Commerce] Upsell suggestion failed (non-blocking)', { error: String(err) }));
    ...
    }).catch(err => logger.warn('[Commerce] Welcome message failed (non-blocking)', { error: String(err) }));
...
export const registerRestockInterestTool = ai.defineTool(
  {
    ...
    inputSchema: StoreContext.extend({
      productQuery: z.string().describe('Name (or partial name) of the product the customer is interested in'),
      customerPhone: z.string(),
  ...
  async ({ companyId, storeId, productQuery, customerPhone, customerName }) => {
    ...
    if (!product) return { success: false, message: `Aucun produit "${productQuery}" trouvé dans le catalogue.` };
    ...
    logger.info('[Commerce] Restock interest registered', {
 ...
 */
export async function notifyRestockInterests(
  ...
  if (!interestsSnap || interestsSnap.empty) return;
  ...
  const { sendMarketing, recordMarketingEvent } = await import('../services/whatsapp/smartOutbound');

  for (const doc of interestsSnap.docs) {
    const interest = doc.data() as { customerPhone: string; customerName?: string | null };
  ...
  logger.info('[Commerce] Restock notifications fired', {
...
async function notifyOwnerOfNewOrder(args: {
  ...
  logger.info('[Commerce] Owner notified of new order', {
  ...
  async ({ companyId, storeId, limit }) => {
    ...
    return {
      orders: snap.docs.map(d => {
        ...
        const ca = (o.createdAt as unknown as { toDate?: () => Date })?.toDate?.() ?? o.createdAt;
        return {
          id: d.id,
...
export const markOrderPaidTool = ai.defineTool(
  {
  ...
  async ({ companyId, storeId, orderId }) => {
    const ref = getFirestore().doc(`companies/${companyId}/stores/${storeId}/orders/${orderId}`);
    const snap = await ref.get();
    if (!snap.exists) return { success: false, message: 'Commande introuvable.' };
...
export async function findProductByName(
  companyId: string, storeId: string, query: string,
  ...
  const exact = snap.docs.find(d => (d.data() as Product).name.toLowerCase() === q);
  if (exact) return { id: exact.id, ...(exact.data() as Product) };
  // 2. Substring match
  const partial = snap.docs.find(d => (d.data() as Product).name.toLowerCase().includes(q));
  if (partial) return { id: partial.id, ...(partial.data() as Product) };
  // 3. Word overlap (2+ words match)
  const queryWords = q.split(/\s+/).filter(w => w.length >= 3);
  for (const d of snap.docs) {
    const name = (d.data() as Product).name.toLowerCase();
...
export const updateProductPriceTool = ai.defineTool(
  {
    name: 'commerceUpdateProductPrice',
    description: 'Update the price of a product. OWNER ONLY — requires authenticated owner session.',
    inputSchema: StoreContext.extend({
      productQuery: z.string().describe('Name (or partial name) of the product to update'),
  ...
  async ({ companyId, storeId, productQuery, newPrice }) => {
    const product = await findProductByName(companyId, storeId, productQuery);
    if (!product) return { success: false, message: `Aucun produit trouvé pour "${productQuery}".` };
    const oldPrice = product.price;
    await getFirestore().doc(`companies/${companyId}/stores/${storeId}/products/${product.id}`)
      .update({ price: newPrice, updatedAt: new Date() });
    void syncProductToMeta(companyId, storeId, product.id, {
...
export const updateProductStockTool = ai.defineTool(
  {
  ...
  async ({ companyId, storeId, productQuery, newStock }) => {
    ...
    const newStatus: Product['status'] = newStock <= 0 ? 'out_of_stock' : (product.status === 'out_of_stock' ? 'active' : product.status);
    await getFirestore().doc(`companies/${companyId}/stores/${storeId}/products/${product.id}`)
...
export const updateProductStatusTool = ai.defineTool(
  ...
  async ({ companyId, storeId, productQuery, status }) => {
    ...
    void syncProductToMeta(companyId, storeId, product.id, { ...product, status });
...
export const deleteProductTool = ai.defineTool(
  ...
  async ({ companyId, storeId, productQuery }) => {
    const product = await findProductByName(companyId, storeId, productQuery);
    if (!product) return { success: false, message: `Aucun produit trouvé pour "${productQuery}".` };
    await getFirestore().doc(`companies/${companyId}/stores/${storeId}/products/${product.id}`).delete();
    void deleteProductFromMeta(companyId, product.id);
    return {
      success: true,
      productName: product.name,
...
// ── Owner command parser (used by webhook intercept) ────────────────────────
export interface OwnerCommand {
  type: 'update_price' | 'update_stock' | 'archive_product' | 'reactivate_product' | 'delete_product' | 'create_product';
  productQuery: string;
  newValue?: number;
  productPrice?: number;
  productStock?: number;
...
export function parseOwnerCommand(text: string): OwnerCommand | null {
  ...
  if (!m) m = /^(?:met|mets|mettre)\s+(?:le\s+)?stock\s+(?:de\s+)?(.+?)\s+(?:à|a)\s+(\d+)$/i.exec(t);
  if (m) {
  ...
  if (m) return { type: 'archive_product', productQuery: m[1].trim() };

  m = /^(?:archive|archiver|cache|masque)\s+(.+?)$/i.exec(t);
  if (m) return { type: 'archive_product', productQuery: m[1].trim() };

  m = /^(?:réactive|reactive|active|disponible|remets)\s+(.+?)$/i.exec(t);
  if (m) return { type: 'reactivate_product', productQuery: m[1].trim() };

  m = /^(?:supprime|supprimer|delete|enlève|enleve|retire)\s+(.+?)$/i.exec(t);
  if (m) return { type: 'delete_product', productQuery: m[1].trim() };

  return null;
}

export async function executeOwnerCommand(
  companyId: string, storeId: string, cmd: OwnerCommand,
): Promise<string> {
  switch (cmd.type) {
    case 'create_product': {
      const db = getFirestore();
      const storeSnap = await db.doc(`companies/${companyId}/stores/${storeId}`).get();
      const currency = (storeSnap.data() as Store | undefined)?.currency ?? 'XOF';
      ...
      const productDoc = {
        ...
        createdBy: 'whatsapp' as const,
        createdAt: new Date(),
        updatedAt: new Date(),
      };
      await ref.set(productDoc);
      void syncProductToMeta(companyId, storeId, ref.id, productDoc);
      logger.info('[Commerce] Product created via text command', {
        companyId, storeId, productId: ref.id, name: cmd.productQuery, price: cmd.productPrice, stock,
      });
      return (
        `✅ *Produit créé* : ${cmd.productQuery} — ${cmd.productPrice!.toLocaleString('fr-FR')} ${currency}\n` +
        `📦 Stock : ${stock}\n\n` +
    ...
    case 'update_price': {
      ...
      if (!product) return `❌ Aucun produit trouvé pour "${cmd.productQuery}".`;
      await getFirestore().doc(`companies/${companyId}/stores/${storeId}/products/${product.id}`)
        .update({ price: cmd.newValue, updatedAt: new Date() });
      void syncProductToMeta(companyId, storeId, product.id, { ...product, price: cmd.newValue! });
      return `✅ Prix de *${product.name}* : ${product.price.toLocaleString('fr-FR')} → ${cmd.newValue!.toLocaleString('fr-FR')} ${product.currency}.`;
    }
    case 'update_stock': {
      const product = await findProductByName(companyId, storeId, cmd.productQuery);
      if (!product) return `❌ Aucun produit trouvé pour "${cmd.productQuery}".`;
      const newStatus: Product['status'] = cmd.newValue! <= 0 ? 'out_of_stock' : 'active';
      await getFirestore().doc(`companies/${companyId}/stores/${storeId}/products/${product.id}`)
        .update({ stockQty: cmd.newValue, status: newStatus, updatedAt: new Date() });
      void syncProductToMeta(companyId, storeId, product.id, { ...product, stockQty: cmd.newValue!, status: newStatus });
    ...
    case 'archive_product': {
      ...
      void syncProductToMeta(companyId, storeId, product.id, { ...product, status: 'archived' });
    ...
    case 'reactivate_product': {
      ...
      void syncProductToMeta(companyId, storeId, product.id, { ...product, status: 'active' });
      if (product.status !== 'active') void notifyRestockInterests(companyId, storeId, product.id);
...
export function isMutatingBusinessAction(text: string): boolean {
  if (!text) return false;
  const t = text.toLowerCase();
  const mutationVerbs = [
...
export function detectStoreBusinessType(text: string): string | null {
  if (!text) return null;
  const t = text.toLowerCase();
  // Order matters: more specific patterns first
  if (/\b(restaurant|resto|menu|plat|cuisine|repas)\b/.test(t)) return 'restaurant';
  if (/\b(h[oô]tel|chambre|s[eé]jour|nuit[eé]e|reservation\s+chambre)\b/.test(t)) return 'hotel';
  if (/\b(salon|coiffure|beaut[eé]|esth[eé]tique|manucure|massage)\b/.test(t)) return 'service';
  if (/\b(cabinet|patient|consultation|m[eé]dical|m[eé]decin|sant[eé])\b/.test(t)) return 'health';
  if (/\b(bien|appartement|maison|villa|immobilier|location|terrain|visite\s+immo)\b/.test(t)) return 'realestate';
  if (/\b(boutique|shop|magasin|article|catalogue)\b/.test(t)) return 'boutique';
  return null;
}
...
export async function findAllStoresByOwnerPhone(
  fromPhone: string,
  companyId: string,
): Promise<Array<{ storeId: string; store: Store }>> {
  const digits = normalizePhone(fromPhone);
  if (!digits || !companyId) return [];
  const db = getFirestore();
  const allSnap = await db.collection(`companies/${companyId}/stores`).get().catch(() => null);
  if (!allSnap || allSnap.empty) return [];
  const out: Array<{ storeId: string; store: Store }> = [];
  for (const doc of allSnap.docs) {
    const data = doc.data() as Store;
    const storedDigits = normalizePhone(data.ownerPhone ?? '');
    if (storedDigits && storedDigits === digits) {
...
export function detectOrlodeMention(
  text: string,
  opts?: { companyName?: string; storeSlugs?: string[] },
): { matched: boolean; stripped: string } {
  if (!text) return { matched: false, stripped: text };
  const trimmed = text.trimStart();
  if (!trimmed.startsWith('@')) return { matched: false, stripped: text };
  // Extract the @token (alphanum + dash + underscore)
  const m = trimmed.match(/^@([a-z0-9][a-z0-9_\-]*)\s*/i);
  if (!m) return { matched: false, stripped: text };
...
async function notifyOwnerOfLowStock(args: {
  ...
  logger.info('[Commerce] Low stock alert sent', { companyId: args.companyId, storeId: args.storeId, count: lowItems.length });
}
...
async function bridgeCustomerToLeads(args: {
  ...
  orderNumber: string;
  currency: string;
  loyaltyPointsDelta?: number; // earned - redeemed; can be negative
}): Promise<void> {
  ...
  logger.info('[Commerce] Customer bridged to leads', {
...
async function sendWelcomeIfFirstOrder(args: {
  ...
  if (snap.exists && (snap.data() as { welcomeSentAt?: Date } | undefined)?.welcomeSentAt) return;

  const { sendMarketing, recordMarketingEvent } = await import('../services/whatsapp/smartOutbound');
  const { generatePromoCode } = await import('../services/whatsapp/marketingTemplates');
  ...
  logger.info('[Commerce] Welcome message dispatched', {
...
export async function proposeUpsellAfterOrder(args: {
  companyId: string;
  storeId: string;
  customerName: string;
  customerPhone: string;
  currency: string;
  justBoughtProductIds: string[];
}): Promise<void> {
  ...
  if (candidates.length < 2 || all.length < 3) return; // catalog trop petit

  const justBought = all.filter(p => args.justBoughtProductIds.includes(p.id));
  if (justBought.length === 0) return;

  // Trim catalog so prompt stays small
  const catalogForPrompt = candidates.slice(0, 30).map(p => ({
    id: p.id,
    name: p.name,
    category: (p as Product & { category?: string }).category ?? null,
    price: p.price,
  }));

  let pickedProduct: (Product & { id: string }) | null = null;
  try {
    const { text } = await ai.generate({
      model: GEMINI_FLASH,
      prompt: `Tu es un vendeur expert. Un client vient d'acheter :
${justBought.map(p => `- ${p.name} (${(p as Product & { category?: string }).category ?? 'non catégorisé'})`).join('\n')}
...
{"productId": "<id ou null>", "reason": "courte raison en français (max 12 mots)"}`,
    ...
    logger.warn('[Commerce] Upsell Gemini call failed', { error: String(err) });
    return;
  }
  ...
  const msg =
    `🎁 *${firstName}*, beaucoup de clients qui achètent "${justBought[0].name}" prennent aussi :\n\n` +
  ...
  logger.info('[Commerce] Upsell sent', {
...
export async function runCommerceDailyJob(): Promise<{ digestsSent: number; reactivationsSent: number }> {
  const db = getFirestore();
  const { whatsappService } = await import('../services/whatsapp/whatsappService');
  const { sendMarketing, shouldSendOnce, recordMarketingEvent } = await import('../services/whatsapp/smartOutbound');
  const { generatePromoCode } = await import('../services/whatsapp/marketingTemplates');
  ...
  for (const storeDoc of storesSnap.docs) {
    ...
    if (!parent || !store.ownerPhone) continue;
    ...
    try {
      ...
      for (const od of ordersSnap.docs) {
        ...
        if (o.paymentStatus === 'paid') revenuePaid += o.total ?? 0;
      ...
      logger.warn('[Commerce] Digest failed for store', { companyId, storeId, error: String(err) });
    }
    ...
    try {
      ...
      if (dueSnap) {
        for (const od of dueSnap.docs) {
          const o = od.data() as Order & { reviewRequestSentAt?: Date };
          if (o.reviewRequestSentAt) continue;
      ...
      logger.warn('[Commerce] Review request failed for store', { companyId, storeId, error: String(err) });
    }
    ...
    try {
      ...
      if (recentSnap) {
        for (const od of recentSnap.docs) {
          const o = od.data() as Order & { followUpSentAt?: Date };
          if (o.followUpSentAt) continue;
          const firstName = o.customerName.split(/\s+/)[0] ?? o.customerName;
      ...
      logger.warn('[Commerce] Reactivation failed for store', { companyId, storeId, error: String(err) });
    }
    ...
    try {
      ...
      if (dormantSnap) {
        for (const cd of dormantSnap.docs) {
          const c = cd.data() as { name: string; phone: string };
      ...
      logger.warn('[Commerce] Win-back failed for store', { companyId, storeId, error: String(err) });
    }
  }

  logger.info('[Commerce] Daily job complete', { digestsSent, reactivationsSent });
...
export async function runReservationReminders(): Promise<{ scanned: number; sent: number; errors: number }> {
  const db = getFirestore();
  const { whatsappService } = await import('../services/whatsapp/whatsappService');
  const now = new Date();
    ...
    for (const r of snap.docs) {
      ...
      try {
        const data = r.data() as {
          customerPhone?: string; customerName?: string; time?: string;
          partySize?: number; remindersSent?: string[];
        };
        ...
        if (sentList.includes(kind)) continue;
        ...
        if (kind === 'two_hours' && data.time) {
          ...
          if (delta < 90 || delta > 150) continue;
        }
        ...
        const store = storeDoc.data() as Store | undefined;
        const companyId = storeRef.parent.parent?.id;
        if (!store || !companyId) continue;
        const cfg = await whatsappService.getConfig(companyId).catch(() => null);
        if (!cfg) continue;
        ...
        errors++;
        logger.warn('[Commerce] Reservation reminder failed', { err: String(err) });
      }
    }
  }
  logger.info('[Commerce] Reservation reminders done', { scanned, sent, errors });
...
export async function runStayCycleMessages(): Promise<{ preSent: number; postSent: number; errors: number }> {
  const db = getFirestore();
  const { whatsappService } = await import('../services/whatsapp/whatsappService');
  const now = new Date();
  const hour = now.getHours();
  ...
  if (hour >= 17 && hour <= 20) {
    ...
    for (const r of snap?.docs ?? []) {
      try {
        const data = r.data() as {
          customerPhone?: string; customerName?: string; nights?: number;
          checkOutDate?: string; stayMessagesSent?: string[];
        };
        if (!data.customerPhone || (data.nights ?? 0) < 1) continue;
        const sentList = Array.isArray(data.stayMessagesSent) ? data.stayMessagesSent : [];
        if (sentList.includes('pre_arrival')) continue;
        const storeRef = r.ref.parent.parent;
        if (!storeRef) continue;
        const storeDoc = await storeRef.get();
        const store = storeDoc.data() as Store & {
          checkInInstructions?: string; houseRules?: string;
          ownerPhone?: string;
        } | undefined;
        const companyId = storeRef.parent.parent?.id;
        if (!store || !companyId) continue;
        // Only hotel/residence packs
        if (store.businessType !== 'hotel' && store.businessType !== 'residence') continue;
        const cfg = await whatsappService.getConfig(companyId).catch(() => null);
        if (!cfg) continue;
        ...
        if (data.nights) lines.push(`📅 ${data.nights} nuit${data.nights > 1 ? 's' : ''} (jusqu'au ${data.checkOutDate ?? '?'})`);
        ...
        errors++;
        logger.warn('[Commerce] Pre-arrival msg failed', { err: String(err) });
      }
    }
  }
    ...
    for (const r of snap?.docs ?? []) {
      try {
        const data = r.data() as {
          ...
          stayMessagesSent?: string[]; status?: string;
        };
        if (!data.customerPhone) continue;
        if (data.status === 'cancelled' || data.status === 'no_show') continue;
        const sentList = Array.isArray(data.stayMessagesSent) ? data.stayMessagesSent : [];
        if (sentList.includes('post_stay')) continue;
        const storeRef = r.ref.parent.parent;
        if (!storeRef) continue;
        const storeDoc = await storeRef.get();
        const store = storeDoc.data() as Store | undefined;
        ...
        const publicUrl = store.slug
          ...
          : null;
        const text =
          `Merci ${greet} d'avoir choisi *${store.name}* 🙏\n\n` +
        ...
        errors++;
        logger.warn('[Commerce] Post-stay msg failed', { err: String(err) });
      }
    }
  }

  logger.info('[Commerce] Stay-cycle done', { preSent, postSent, errors });
...
export async function runMorningBrief(): Promise<{ briefsSent: number; errors: number }> {
  const db = getFirestore();
  const { whatsappService } = await import('../services/whatsapp/whatsappService');
  const storesSnap = await db.collectionGroup('stores')
    .where('status', '==', 'active').limit(500).get();
  ...
  for (const storeDoc of storesSnap.docs) {
    ...
    if (store.businessType && !['restaurant', 'boutique', 'service', 'health', 'hotel', 'residence'].includes(store.businessType)) continue;
    ...
    try {
      ...
      for (const o of ordersSnap?.docs ?? []) {
        const data = o.data() as Order;
        if (data.paymentStatus === 'paid') revenue += data.total ?? 0;
        for (const it of (data.items ?? [])) itemCounts[it.name] = (itemCounts[it.name] ?? 0) + it.qty;
      }
      ...
      for (const r of resoSnap?.docs ?? []) {
        covers += (r.data() as { partySize?: number }).partySize ?? 0;
      }
      ...
      const lowStock = productsSnap?.docs.map(d => (d.data() as { name?: string }).name).filter(Boolean) ?? [];

      // Skip silently if zero activity AND no reservation
      if (ordersCount === 0 && resoCount === 0 && lowStock.length === 0) continue;

      const currency = store.currency ?? 'XOF';
      const fmt = (n: number) => `${n.toLocaleString('fr-FR')} ${currency}`;
      const verticalLabel =
        store.businessType === 'hotel' ? 'ton hôtel' :
      ...
      if (revenue > 0) lines.push(`• CA payé : *${fmt(revenue)}*`);
      if (topEntry) lines.push(`• Star : ${topEntry[0]} (${topEntry[1]} ventes)`);
      lines.push('');
      ...
      errors++;
      logger.warn('[Commerce] Morning brief failed', { storeId, err: String(err) });
    }
  }
  logger.info('[Commerce] Morning brief done', { briefsSent, errors });
...
export async function runStockAlerts(): Promise<{ scanned: number; alerted: number; errors: number }> {
  const db = getFirestore();
  const { whatsappService } = await import('../services/whatsapp/whatsappService');
  ...
  if (!lowSnap) return { scanned: 0, alerted: 0, errors: 0 };

  let alerted = 0;
  let errors = 0;
  for (const p of lowSnap.docs) {
    try {
      const data = p.data() as { name?: string; stockQty?: number; lastStockAlertDate?: string };
      if (data.lastStockAlertDate === today) continue; // already alerted today
      ...
      errors++;
      logger.warn('[Commerce] Stock alert failed', { err: String(err) });
    }
  }
  logger.info('[Commerce] Stock alerts done', { scanned: lowSnap.size, alerted, errors });
...
export const createReservationTool = ai.defineTool(
  {
    ...
    inputSchema: StoreContext.extend({
      ...
      notes: z.string().optional().describe('Allergies, occasion, demandes spéciales'),
    }),
    outputSchema: z.object({
      success: z.boolean(),
  ...
  async ({ companyId, storeId, customerName, customerPhone, date, time, partySize, notes }) => {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) return { success: false, message: 'Format date invalide. Utilise YYYY-MM-DD.' };
    if (!/^\d{1,2}:\d{2}$/.test(time)) return { success: false, message: 'Format heure invalide. Utilise HH:mm.' };
    const db = getFirestore();
    ...
    const storePre = storePreSnap.data() as Store | undefined;
    const hoursCheck = isStoreOpenAt({ openingHours: storePre?.openingHours, date, time: time.padStart(5, '0') });
    if (hoursCheck.status === 'closed') {
...
export const createBookingTool = ai.defineTool(
  {
    ...
    inputSchema: StoreContext.extend({
      ...
      checkOutDate: z.string().describe('YYYY-MM-DD — must be after checkInDate'),
      guests: z.number().int().positive(),
      roomId: z.string().optional().describe('Optional specific room id — leave empty if customer accepts owner choice'),
  ...
  async ({ companyId, storeId, customerName, customerPhone, checkInDate, checkOutDate, guests, roomId, notes }) => {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(checkInDate)) return { success: false, message: 'Format date d\'arrivée invalide (YYYY-MM-DD).' };
    if (!/^\d{4}-\d{2}-\d{2}$/.test(checkOutDate)) return { success: false, message: 'Format date de départ invalide (YYYY-MM-DD).' };
    if (checkOutDate <= checkInDate) return { success: false, message: 'La date de départ doit être après la date d\'arrivée.' };

    // Anti-overbooking against external calendars (Booking/Airbnb/Google iCal)
    const externalConflict = await findExternalBlockConflict({
...
export const bookAppointmentTool = ai.defineTool(
  {
    ...
    inputSchema: StoreContext.extend({
      customerName: z.string(),
      customerPhone: z.string(),
      serviceQuery: z.string().describe('Service name (matched to catalog) — e.g. "coupe homme", "manucure"'),
      date: z.string().describe('YYYY-MM-DD'),
      time: z.string().describe('HH:mm 24h'),
      practitionerName: z.string().optional().describe('Optional preferred practitioner — leave empty if no preference'),
      notes: z.string().optional(),
    }),
    outputSchema: z.object({
      success: z.boolean(),
  ...
  async ({ companyId, storeId, customerName, customerPhone, serviceQuery, date, time, practitionerName, notes }) => {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) return { success: false, message: 'Format date invalide (YYYY-MM-DD).' };
    if (!/^\d{1,2}:\d{2}$/.test(time)) return { success: false, message: 'Format heure invalide (HH:mm).' };

    // Refuse out-of-hours appointments
    {
      const sSnap = await getFirestore().doc(`companies/${companyId}/stores/${storeId}`).get();
      const s = sSnap.data() as Store | undefined;
      const h = isStoreOpenAt({ openingHours: s?.openingHours, date, time: time.padStart(5, '0') });
      if (h.status === 'closed') {
    ...
    const durationMinutes = (service as Product & { durationMinutes?: number }).durationMinutes ?? 30;
...
export const bookConsultationTool = ai.defineTool(
  {
    ...
    inputSchema: StoreContext.extend({
      ...
      date: z.string().describe('YYYY-MM-DD'),
      time: z.string().describe('HH:mm 24h'),
      reason: z.string().describe('Motif de la consultation (ex: "mal de tête", "contrôle annuel", "vaccin")'),
      practitionerName: z.string().optional().describe('Médecin/praticien souhaité — optionnel'),
      durationMinutes: z.number().optional().describe('Durée prévue (par défaut 30 min)'),
    }),
    outputSchema: z.object({
      success: z.boolean(),
      reservationId: z.string().optional(),
...
export const bookViewingTool = ai.defineTool(
  {
    ...
    inputSchema: StoreContext.extend({
      customerName: z.string(),
      customerPhone: z.string(),
      propertyQuery: z.string().describe('Property name/reference (matched to listings) — e.g. "appartement Cocody T3", "villa Riviera"'),
      date: z.string().describe('YYYY-MM-DD'),
      time: z.string().describe('HH:mm 24h'),
      agentName: z.string().optional().describe('Agent immobilier souhaité — optionnel'),
      notes: z.string().optional(),
    }),
    outputSchema: z.object({
      success: z.boolean(),
...
export const listMyStoresTool = ai.defineTool(
  {
    ...
    outputSchema: z.object({
      ...
      stores: z.array(z.object({
        ...
        items: z.number().optional(),  // count of products / rooms
  ...
  async ({ companyId }) => {
    ...
    const stores = await Promise.all(snap.docs.map(async d => {
      const data = d.data() as { name?: string; slug?: string; businessType?: string; currency?: string; status?: string };
    ...
    for (const s of stores) counts[s.businessType] = (counts[s.businessType] ?? 0) + 1;
...
export const storeSummaryTool = ai.defineTool(
  {
    ...
    inputSchema: z.object({
      companyId: z.string(),
      storeId: z.string().optional().describe('Optional. If absent, summarize the FIRST store of the company.'),
    }),
    outputSchema: z.object({
      success: z.boolean(),
      ...
      reservationsThisMonth: z.number(),
      revenueThisMonth: z.number(),
      currency: z.string(),
      topItems: z.array(z.object({ name: z.string(), price: z.number().optional(), stockQty: z.number().optional() })),
      message: z.string(),
    }),
  },
  async ({ companyId, storeId }) => {
    ...
    if (!sId) {
      ...
      if (snap.empty) return { success: false, itemsCount: 0, activeItemsCount: 0, reservationsThisMonth: 0, revenueThisMonth: 0, currency: 'XOF', topItems: [], message: 'Aucun store dans cette company.' };
      sId = snap.docs[0].id; storeData = snap.docs[0].data();
    } else {
      const snap = await db.doc(`companies/${companyId}/stores/${sId}`).get();
      if (!snap.exists) return { success: false, itemsCount: 0, activeItemsCount: 0, reservationsThisMonth: 0, revenueThisMonth: 0, currency: 'XOF', topItems: [], message: 'Store introuvable.' };
      storeData = snap.data() ?? {};
    }
    const bt = (storeData['businessType'] as string) ?? 'boutique';
    ...
    const topItems = items
      .map(i => ({
        name: (i['name'] as string) ?? (i['number'] as string) ?? '(sans nom)',
    ...
    let resInMonth = 0; let revenueInMonth = 0;
    for (const r of resSnap.docs) {
      const data = r.data() as { createdAt?: { toDate?: () => Date }; totalAmount?: number };
      const d = data.createdAt?.toDate?.();
      if (d && d >= monthStart) {
        resInMonth++;
        if (typeof data.totalAmount === 'number') revenueInMonth += data.totalAmount;
      }
    }

    const message =
      `📊 *${storeData['name'] ?? 'Store'}* (${bt})\n` +
...
export const analyzeMyBusinessTool = ai.defineTool(
  {
    ...
    outputSchema: z.object({
      ...
      insights: z.array(z.object({
        store: z.string(),
        type: z.enum(['warning', 'opportunity', 'positive']),
  ...
  async ({ companyId }) => {
    ...
    const insights: Array<{ store: string; type: 'warning' | 'opportunity' | 'positive'; observation: string; suggestion: string }> = [];

    const monthStart = new Date(); monthStart.setDate(1); monthStart.setHours(0, 0, 0, 0);

    for (const sDoc of storesSnap.docs) {
      const sd = sDoc.data() as Record<string, unknown>;
      const bt = (sd['businessType'] as string) ?? 'boutique';
      const name = (sd['name'] as string) ?? sDoc.id;
      const itemsCol = bt === 'hotel' ? 'rooms' : bt === 'health' ? 'patients' : 'products';
      const [itemsSnap, resSnap] = await Promise.all([
      ...
      for (const r of resSnap.docs) {
        const data = r.data() as { createdAt?: { toDate?: () => Date }; customerPhone?: string };
        const d = data.createdAt?.toDate?.();
        if (d && d >= monthStart) {
          resInMonth++;
          if (data.customerPhone) phones.add(data.customerPhone);
        }
      }
...
export const listPractitionersTool = ai.defineTool(
  {
  ...
  async ({ companyId, storeId }) => {
    try {
      const snap = await getFirestore().doc(`companies/${companyId}/stores/${storeId}`).get().catch(() => null);
      const data = snap?.data() as { practitioners?: Practitioner[] } | undefined;
      ...
      logger.error('[Commerce] listPractitioners failed', { error: String(err) });
...
export const COMMERCE_OWNER_TOOLS = [
  getOrdersTool, markOrderPaidTool,
  updateProductPriceTool, updateProductStockTool, updateProductStatusTool, deleteProductTool,
  listMyStoresTool, storeSummaryTool, analyzeMyBusinessTool,
];
export const COMMERCE_PUBLIC_TOOLS = [listProductsTool, placeOrderTool, registerRestockInterestTool, createReservationTool, createBookingTool, bookAppointmentTool, bookConsultationTool, bookViewingTool, listPractitionersTool];
export const COMMERCE_TOOLS = [...COMMERCE_PUBLIC_TOOLS, ...COMMERCE_OWNER_TOOLS];
