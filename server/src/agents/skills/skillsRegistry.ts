...
export type SkillPlan = 'starter' | 'business' | 'enterprise';
export type AgentName =
  | 'documents' | 'qa' | 'meeting' | 'vision' | 'insights' | 'comms'
  | 'reception' | 'hr' | 'accounting' | 'sales' | 'support'
  | 'legal' | 'training' | 'it' | 'cybersecurity' | 'marketing' | 'wildcard';
...
export function getSkillsByAgent(agent: AgentName): Skill[] {
  return SKILLS_REGISTRY.filter((s) => s.agent === agent);
}

export function getSkillsByPlan(plan: SkillPlan): Skill[] {
  const planOrder: Record<SkillPlan, number> = { starter: 0, business: 1, enterprise: 2 };
  return SKILLS_REGISTRY.filter((s) => planOrder[s.plan] <= planOrder[plan]);
}
...
export const SKILLS_SUMMARY = {
  ...
  byPlan: {
    starter:    SKILLS_REGISTRY.filter((s) => s.plan === 'starter').length,
    business:   SKILLS_REGISTRY.filter((s) => s.plan === 'business').length,
    enterprise: SKILLS_REGISTRY.filter((s) => s.plan === 'enterprise').length,
  },
  byAgent: Object.fromEntries(
    Array.from(new Set(SKILLS_REGISTRY.map((s) => s.agent))).map((a) => [
      a,
      SKILLS_REGISTRY.filter((s) => s.agent === a).length,
    ])
  ),
};
