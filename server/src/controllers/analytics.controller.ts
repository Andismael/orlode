...
export async function getOverview(req: AuthenticatedRequest, res: Response): Promise<void> {
  ...
  const totalStorageBytes = documents.reduce((s, d) => s + ((d['fileSize'] as number | undefined) ?? 0), 0);
  ...
  res.json({
    ...
    data: {
      documents: {
        ...
        totalChunks: documents.reduce((s, d) => s + ((d['chunksCreated'] as number | undefined) ?? 0), 0),
        totalStorageBytes,
        storageGB,
      },
      conversations: {
        total:         conversations.length,
        totalMessages: conversations.reduce((s, c) => s + ((c['messageCount'] as number | undefined) ?? 0), 0),
      },
      meetings: {
        total:        meetings.length,
        transcribed:  meetings.filter((m) => m['hasTranscript']).length,
        totalMinutes: meetings.reduce((s, m) => s + ((m['duration'] as number | undefined) ?? 0), 0),
      },
      companyId,
    },
  });
...
export async function getCharts(req: AuthenticatedRequest, res: Response): Promise<void> {
  const companyId = (req.query['companyId'] as string | undefined) ?? req.user?.companyId;
  if (!companyId) throw new AppError('Company ID required', 400);
  ...
  conversationsSnap.docs.forEach((doc) => {
    ...
    if (createdAt >= since) {
      ...
      dayMap.set(key, {
        ...
        messages: prev.messages + ((data['messageCount'] as number | undefined) ?? 0),
      });
    }
  });
  ...
  documentsSnap.docs.forEach((doc) => {
    const raw = ((doc.data()['fileType'] as string | undefined) ?? 'other').toLowerCase().replace('.', '');
  ...
  conversationsSnap.docs.forEach((doc) => {
    const title = ((doc.data()['title'] as string | undefined) ?? '').toLowerCase();
    let matched = false;
    ...
    if (!matched) topicCount.set('Other', (topicCount.get('Other') ?? 0) + 1);
  });
...
export async function getActivity(req: AuthenticatedRequest, res: Response): Promise<void> {
  ...
  type ActivityItem = {
    ...
    description: string; timestamp: string; user?: string;
  };
  ...
  docsSnap.docs.forEach((doc) => {
    const d = doc.data();
    const isNew = (d['status'] as string) === 'processing' || (d['status'] as string) === 'pending';
    activities.push({
      id:          `doc-${doc.id}`,
      type:        isNew ? 'upload' : 'document',
      title:       isNew ? 'Document uploaded' : 'Document indexed',
      description: `${d['originalName'] as string} — ${(d['chunksCreated'] as number | undefined) ?? 0} chunks`,
      timestamp:   toDate(d['uploadedAt']).toISOString(),
      user:        d['uploadedBy'] as string | undefined,
    });
  });
  ...
  convsSnap.docs.forEach((doc) => {
    ...
    activities.push({
      ...
      description: (d['title'] as string | undefined) ?? 'Chat session',
  ...
  meetingsSnap.docs.forEach((doc) => {
    ...
    activities.push({
      id:          `meet-${doc.id}`,
      type:        'meeting',
      title:       (d['hasTranscript'] as boolean) ? 'Meeting transcribed' : 'Meeting scheduled',
      description: (d['title'] as string | undefined) ?? 'Meeting',
      timestamp:   toDate(d['updatedAt']).toISOString(),
    });
  });
