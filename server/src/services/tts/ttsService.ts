...
interface TTSOptions {
  text: string;
  language?: string;       // 'fr-FR', 'en-US', 'ar-XA', etc.
 ...
 */
async function googleTTS(text: string, language: string, rate: number): Promise<Buffer> {
  ...
  if (!response.ok) {
    ...
    logger.error('[TTS/Google] API error', { status: response.status, body: errorBody });
    ...
    const fallbackData = await fallbackResponse.json() as { audioContent: string };
 ...
 */
async function elevenLabsTTS(text: string, _language: string, rate: number): Promise<Buffer> {
  const apiKey = env.ELEVENLABS_API_KEY;
  const voiceId = env.ELEVENLABS_VOICE_ID || 'EXAVITQu4vr4xnSDxMaL'; // Default: Sarah
...
export async function synthesizeSpeech(options: TTSOptions): Promise<TTSResult> {
  ...
  if (provider === 'elevenlabs' && !env.ELEVENLABS_API_KEY) {
    logger.warn('[TTS] ElevenLabs requested but not configured, falling back to Google');
    provider = 'google';
  }

  // Si Google pas configuré mais ElevenLabs oui, switch
  if (provider === 'google' && !env.GOOGLE_AI_API_KEY && env.ELEVENLABS_API_KEY) {
    logger.warn('[TTS] Google not configured, falling back to ElevenLabs');
    provider = 'elevenlabs';
  }

  logger.info('[TTS] Synthesizing', { provider, language, textLength: text.length });
