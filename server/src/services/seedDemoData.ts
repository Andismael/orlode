...
function futureDate(daysAhead: number) { const d = new Date(); d.setDate(d.getDate() + daysAhead); return d; }
function todayStr() { return new Date().toISOString().split('T')[0]; }

export async function seedDemoData(companyId: string): Promise<{ seeded: string[]; counts: Record<string, number> }> {
  ...
  if (companyDoc.data()?.['demoSeeded']) {
    logger.info(`[SeedDemo] Already seeded — clearing first...`);
    await clearDemoData(companyId);
  }

  logger.info(`[SeedDemo] Seeding demo data for company ${companyId}`);

  // ── 1. EMPLOYEES (users collection) ────────────────────────────────────
  const employees: { uid: string; name: string; email: string; dept: string; role: string }[] = [];
  for (let i = 0; i < 12; i++) {
    const first = FIRST_NAMES[i];
    const last = pick(LAST_NAMES);
    const uid = `demo_${generateId().slice(0, 8)}`;
  ...
  for (const v of visitorData) {
    const checkIn = new Date(); checkIn.setHours(rand(8, 14), rand(0, 59));
    ...
    await db.collection('visitors').doc(vid).set({
      ...
      estimatedWait: (v as { estimatedWait?: number }).estimatedWait ?? null,
      createdAt: checkIn,
    });
    // Timeline
    await db.collection(`visitors/${vid}/timeline`).doc(generateId()).set({ action: 'Check-in', details: `${v.name} enregistre a la reception`, user: 'reception', timestamp: checkIn });
    if (v.hostResponse) {
      await db.collection(`visitors/${vid}/timeline`).doc(generateId()).set({ action: `Reponse host: ${v.hostResponseMessage}`, details: v.hostResponseMessage, user: 'host', timestamp: new Date(checkIn.getTime() + 120000) });
    }
  }
  ...
  seeded.push('preRegistrations'); counts['preRegistrations'] = preRegs.length;
  ...
  for (const d of deliveries) {
    const id = generateId();
    const recvAt = new Date(); recvAt.setHours(rand(8, 12), rand(0, 59));
  ...
  seeded.push('deliveries'); counts['deliveries'] = deliveries.length;
  ...
  for (const a of appointments) {
    ...
    await db.collection('appointments').doc(generateId()).set({
      companyId, ...a, scheduledAt: d, location: pick(['Salle A', 'Salle B', 'Bureau Direction', 'Visio']),
      createdAt: pastDate(rand(1, 7)),
    });
  }
  seeded.push('appointments'); counts['appointments'] = appointments.length;
  ...
  seeded.push('parkingSpots'); counts['parkingSpots'] = 8;
  ...
  seeded.push('phishingCampaigns'); counts['phishingCampaigns'] = phishCampaigns.length;
  ...
  seeded.push('securityPolicies'); counts['securityPolicies'] = policies.length;
  ...
  for (let i = 0; i < 5; i++) {
    ...
    await db.collection('leaveRequests').doc(generateId()).set({
  ...
  for (let i = 0; i < 5; i++) {
    await db.collection('supportTickets').doc(generateId()).set({
  ...
  for (let i = 0; i < 4; i++) {
    await db.collection('itTickets').doc(generateId()).set({
  ...
  for (let i = 0; i < 5; i++) {
    await db.collection('marketingPosts').doc(generateId()).set({
  ...
  for (let i = 0; i < 4; i++) {
    await db.collection('trainingCourses').doc(generateId()).set({
  ...
  for (const inc of secIncidents) {
    ...
    await db.collection(`companies/${companyId}/securityIncidents/${id}/timeline`).doc(generateId()).set({
      action: 'Incident detecte', status: 'detected', user: 'system', details: inc.description, timestamp: d,
    });
  }
  seeded.push('securityIncidents'); counts['securityIncidents'] = secIncidents.length;
  ...
  seeded.push('vulnerabilities'); counts['vulnerabilities'] = vulns.length;
  ...
  seeded.push('threatFeed'); counts['threatFeed'] = threats.length;
  ...
  seeded.push('securityScore'); counts['securityScore'] = 1;
  ...
  logger.info(`[SeedDemo] Done: ${seeded.join(', ')} — ${JSON.stringify(counts)}`);
...
export async function clearDemoData(companyId: string): Promise<void> {
  ...
  for (const doc of demoUsers.docs) {
    if (doc.id.startsWith('demo_')) await doc.ref.delete();
  }
  ...
  for (const doc of usageSnap.docs) await doc.ref.delete();

  await db.collection('companies').doc(companyId).update({ demoSeeded: false });
  logger.info(`[SeedDemo] Cleared demo data for company ${companyId}`);
}
