...
export type VideoModel    = 'wan' | 'kling' | 'veo3' | 'heygen';
export type VideoQuality  = 'budget' | 'standard' | 'premium';
export type AspectRatio   = '16:9' | '9:16' | '1:1';
export type VideoPlatform = 'linkedin' | 'instagram' | 'tiktok' | 'facebook' | 'youtube' | 'whatsapp';
...
const PLATFORM_FORMATS: Record<VideoPlatform, PlatformFormat> = {
...
const MODEL_BY_QUALITY: Record<VideoQuality, { model: VideoModel; costPer10s: number }> = {
...
export class VideoGenerationService {
   ...
   */
  async generateForPlatforms(req: VideoGenerationRequest): Promise<GeneratedVideo[]> {
    ...
    for (const platform of req.platforms) {
      ...
      try {
        ...
        logger.error(`[Video] Failed to generate for ${platform}`, { error: err });
        results.push({
          platform,
          aspectRatio: format.aspectRatio,
          videoUrl:    '',
  ...
  estimateCost(quality: VideoQuality, duration: number, platforms: VideoPlatform[]): number {
  ...
  private async pollKlingTask(taskId: string, apiKey: string): Promise<string> {
    ...
    for (let i = 0; i < MAX_ATTEMPTS; i++) {
      ...
      try {
        ...
        logger.info(`[Video] Kling task ${taskId} — status: ${status} (attempt ${i + 1})`);
        ...
        logger.warn(`[Video] Kling polling error attempt ${i + 1}`, { error: err });
      }
    }
  ...
  private async pollVeo3Operation(operationName: string, apiKey: string): Promise<string> {
    ...
    for (let i = 0; i < MAX_ATTEMPTS; i++) {
      ...
      try {
        ...
        const data = await res.json() as {
          ...
          response?: {
            predictions?: Array<{ bytesBase64Encoded?: string; mimeType?: string; videoUri?: string }>;
          };
          error?: { message?: string };
        };
        logger.info(`[Video] Veo3 operation ${operationName} — done: ${data.done} (attempt ${i + 1})`);
        ...
        logger.warn(`[Video] Veo3 polling error attempt ${i + 1}`, { error: err });
      }
    }
  ...
  private async callVideoAPI(params: {
    model: VideoModel;
    prompt: string;
    aspectRatio: AspectRatio;
    duration: number;
    ...
    switch (params.model) {
      ...
      case 'kling': {
        ...
        if (!apiKey) throw new Error('KLING_API_KEY / PIAPI_API_KEY not configured');
        ...
        if (data.task_id) return await this.pollKlingTask(data.task_id, apiKey);
      ...
      case 'veo3': {
        ...
        if (!data.name) throw new Error('Veo3: no operation name in response');
