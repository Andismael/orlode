...
export async function exchangeGoogleCode(code: string, redirectUri: string): Promise<GoogleTokens> {
  ...
  try {
    ...
    const chJson = await chRes.json() as { items?: Array<{ id: string; snippet?: { title?: string } }> };
    if (chJson.items?.[0]) {
...
export async function uploadVideoToYouTube(args: {
  accessToken: string;
  videoUrl: string;       // public URL of the video (e.g. Firebase Storage)
  title: string;
  description: string;
  tags?: string[];
  ...
  if (!uploadUrl) throw new Error('No Location header returned from YouTube upload init');
  ...
  const putJson = await putRes.json() as { id?: string; error?: { message?: string } };
  if (!putRes.ok || !putJson.id) {
    throw new Error(putJson.error?.message ?? `Upload failed (HTTP ${putRes.status})`);
  }
  logger.info('[YouTube] video uploaded', { videoId: putJson.id, title });
