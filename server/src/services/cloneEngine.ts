...
export interface CloneConfig {
  // Identity
  name: string;              // "Sofia", "Max", "L'Assistant TechCorp"
  role: string;              // sales | support | reception | guide | general
  ...
  // Personality
  tone: string;              // professional | friendly | dynamic | expert | casual
  personality: string;       // Free text: "chaleureux, pedagogue, commercial"
  language: string;          // fr | en | auto

  // Rules
  canDo: string[];           // What the clone CAN do
  cantDo: string[];          // What it CANNOT do (legal limits, competitors, etc.)
  greeting: string;          // Welcome message
  fallbackMessage: string;   // When it can't help

  // Products/Services
  products: string[];        // Key offerings
  uniqueValue: string;       // USP / value proposition
  pricing: string;           // Pricing info (or "contact us")
  ...
  connectToOrchestrator: boolean; // Can it use internal agents?

  // Public storefronts — auto-built from companies/{cid}/stores by businessType.
  // The clone shares the matching link when a visitor asks about products,
  // bookings, the menu, rooms, services, properties, or appointments.
  publicLinks: Array<{
    type: string;            // boutique | restaurant | hotel | service | health | realestate
    label: string;           // "boutique en ligne", "menu", "chambres", ...
    url: string;             // absolute URL
    intent: string;          // when to share it
  }>;

  // Light-touch context per active store — drives the per-vertical
  // "Consultation Mode" prompt block (proactive AI questions before recommending).
  storeContext: Array<{
    type: string;
    name: string;
    establishmentType?: string;  // resto sub-type (restaurant|maquis|bar)
    salonType?: string;          // salon sub-type (coiffure|esthetique|spa|barber)
  }>;
}

// businessType → public path prefix + label + intent
const PUBLIC_LINK_MAP: Record<string, { path: string; label: string; intent: string }> = {
...
export async function getCloneConfig(companyId: string): Promise<CloneConfig> {
  ...
  const publicLinks: CloneConfig['publicLinks'] = [];
  const storeContext: CloneConfig['storeContext'] = [];
  if (storesSnap) {
    for (const doc of storesSnap.docs) {
      ...
      storeContext.push({
        ...
        ...(data['establishmentType'] ? { establishmentType: data['establishmentType'] as string } : {}),
        ...(data['salonType'] ? { salonType: data['salonType'] as string } : {}),
      });
  ...
  return {
    name: (clone['name'] as string) ?? (settings['knowledgeAgentName'] as string) ?? 'Assistant',
    role: (clone['role'] as string) ?? 'general',
    company: (company['name'] as string) ?? 'Mon Entreprise',
    sector: (company['sector'] as string) ?? 'business',
    description: (company['description'] as string) ?? '',
    website: (company['website'] as string) ?? '',
    tone: (clone['tone'] as string) ?? (settings['aiPersonality'] as string) ?? 'professional',
    personality: (clone['personality'] as string) ?? 'professionnel, chaleureux, efficace',
    language: (clone['language'] as string) ?? (settings['language'] as string) ?? 'fr',
    canDo: (clone['canDo'] as string[]) ?? ['repondre aux questions', 'creer des devis', 'prendre des rendez-vous', 'orienter vers le bon service'],
    cantDo: (clone['cantDo'] as string[]) ?? ['donner des conseils juridiques', 'partager des informations confidentielles'],
    greeting: (clone['greeting'] as string) ?? `Bonjour ! Je suis l'assistant de ${(company['name'] as string) ?? 'notre entreprise'}. Comment puis-je vous aider ?`,
    fallbackMessage: (clone['fallbackMessage'] as string) ?? 'Je ne suis pas en mesure de repondre a cette question. Souhaitez-vous etre mis en contact avec un collegue ?',
    products: (clone['products'] as string[]) ?? [],
    uniqueValue: (clone['uniqueValue'] as string) ?? '',
    pricing: (clone['pricing'] as string) ?? 'Contactez-nous pour un devis personnalise.',
    logoUrl: (company['logoUrl'] as string) ?? '',
    primaryColor: (clone['primaryColor'] as string) ?? '#0019FF',
    accentColor: (clone['accentColor'] as string) ?? '#0092FF',
    maxMessagesPerSession: (clone['maxMessagesPerSession'] as number) ?? 50,
    captureLeads: (clone['captureLeads'] as boolean) ?? true,
    autoCreateTickets: (clone['autoCreateTickets'] as boolean) ?? true,
    connectToOrchestrator: (clone['connectToOrchestrator'] as boolean) ?? true,
    publicLinks,
    storeContext,
  };
}
...
function consultationBlockFor(ctx: CloneConfig['storeContext']): string {
...
export function buildClonePrompt(config: CloneConfig): string {
...
- Langue: ${({
  ...
  } as Record<string, string>)[config.language] ?? 'Detecte la langue du message recu et reponds dans cette meme langue. Sois polyvalent (fr, en, ar, es, de, pt, etc.)'}
...
// ══════════════════════════════════════════════════════════════════════════════
// 3. CLONE CHAT (main entry point)
// ══════════════════════════════════════════════════════════════════════════════

export interface CloneChatInput {
  companyId: string;
  message: string;
  sessionId?: string;
  channel?: 'web' | 'widget' | 'whatsapp' | 'telegram' | 'api' | 'voice';
  visitorName?: string;
  visitorEmail?: string;
}

export interface CloneChatOutput {
  reply: string;
  sessionId: string;
  cloneName: string;
  ...
  ctaButtons?: { label: string; action: string; icon: string }[];
  leadScore?: { score: number; intent: string; type: string };
  escalateToHuman?: boolean;
  escalateReason?: string;
  /** When notifyHost was invoked during this turn, the kiosk can poll this ID for the host's reply. */
  hostNotificationId?: string;
}

// Session storage — 2-tier cache: in-memory (hot) + Firestore (persistent across instances)
...
async function loadSession(sessionId: string): Promise<SessionData | null> {
  ...
  if (cached && Date.now() - cached.createdAt < SESSION_TTL) return cached;
  ...
  try {
    ...
    const data = doc.data() as { messages: SessionData['messages']; createdAt: number; leadCaptured: boolean } | undefined;
    if (!data) return null;
...
async function saveSession(sessionId: string, session: SessionData): Promise<void> {
  sessions.set(sessionId, session);
  // Fire-and-forget Firestore save (don't block reply)
  try {
    await getFirestore().collection(FIRESTORE_SESSION_COLLECTION).doc(sessionId).set(
      {
    ...
    logger.warn('[Clone] Session save to Firestore failed (in-memory only)', { sessionId, err: String(err) });
  }
}

/** Main clone chat function */
export async function cloneChat(input: CloneChatInput): Promise<CloneChatOutput> {
  const { companyId, message, channel = 'web', visitorName, visitorEmail } = input;
  const sessionId = input.sessionId || generateId();

  logger.info(`[Clone] Company ${companyId}, session ${sessionId}: "${message.slice(0, 80)}"`);
  ...
  const channelToneOverrides: Record<string, string> = {
...
- Après createAppointment/confirmAppointment success, utilise EXACTEMENT la date et l'heure renvoyées par l'outil` : '';
  ...
  const { CLONE_SAFE_TOOLS } = await import('../agents/tools/cloneTools');
  ...
  let hostNotificationId: string | undefined;
  try {
    for (const msg of response.messages ?? []) {
      const content = (msg as { content?: unknown[] }).content ?? [];
      for (const part of content as Array<Record<string, unknown>>) {
        const tr = part['toolResponse'] as { name?: string; output?: Record<string, unknown> } | undefined;
        if (tr?.name === 'notifyHost' && typeof tr.output?.['notificationId'] === 'string') {
          hostNotificationId = tr.output['notificationId'] as string;
        }
      }
    }
  } catch { /* non-critical */ }
  ...
  for (const trigger of escalationTriggers) {
    if (msgLower.includes(trigger)) { escalateToHuman = true; escalateReason = `Visiteur a demande: "${trigger}"`; break; }
  }
  // Also escalate after many messages without resolution
  if (session.messages.length > 16 && !session.leadCaptured) {
    escalateToHuman = true; escalateReason = 'Conversation longue sans resolution';
  }

  // ── LEAD CAPTURE + SCORING ────────────────────────────────────────────
  let leadCaptured = session.leadCaptured;
  let leadScore: { score: number; intent: string; type: string } | undefined;
  ...
  if (config.captureLeads && !leadCaptured) {
    ...
    if (capturedEmail || capturedPhone) {
      ...
      if (capturedEmail) {
        ...
        if (!existing.empty) existingContactId = existing.docs[0].id;
      }
      ...
      await db.collection(`companies/${companyId}/cloneLeads`).doc(generateId()).set({
      ...
      logger.info(`[Clone] Lead captured (score ${score}, contact ${existingContactId}): ${capturedEmail || capturedPhone}`);
    }
  }
  ...
  if (intentType === 'support' && techKeywords.test(message) && config.connectToOrchestrator) {
    ...
    try {
      ...
      await (cloneToITTool as (a: unknown) => Promise<unknown>)({ companyId, issue: message, customerEmail: visitorEmail, customerName: visitorName, channel });
    } catch {}
  }
  if (intentType === 'support' && config.connectToOrchestrator) {
    try {
      const { autoResolveTool } = await import('../agents/support.agent');
      const resolution = await (autoResolveTool as (a: unknown) => Promise<{ resolved: boolean; solution: string; confidence: number }>)({ companyId, issue: message, customerEmail: visitorEmail ?? '' });
  ...
  const ctaButtons: { label: string; action: string; icon: string }[] = [];
  ...
  saveSession(sessionId, session).catch(() => { /* non-critical */ });
