...
import { loadCompany } from '../hr/contractPdfService';

interface WemasContractData {
...
export async function renderWemasContractPdf(
  companyId: string,
  data: WemasContractData,
): Promise<Buffer> {
  ...
  return await new Promise<Buffer>(async (resolve, reject) => {
    try {
      ...
      if (company.logoUrl) {
        ...
        if (logo) {
          try { doc.image(logo, 50, 45, { width: 70 }); } catch { /* skip */ }
        }
      }
      ...
      let y = 68;
      if (company.legalForm) { doc.text(company.legalForm, 140, y); y += 11; }
      if (company.address)   { doc.text(company.address, 140, y);   y += 11; }
      const contactLine = [company.phone, company.email].filter(Boolean).join(' · ');
      if (contactLine)       { doc.text(contactLine, 140, y);       y += 11; }
      ...
      doc.fontSize(8).fillColor('#94A3A0').moveDown(2)
         .text('Document généré par Orlode AI · Signature sécurisée via Wemas',
               { align: 'center' });
