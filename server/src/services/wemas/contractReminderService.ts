...
async function sendReminderEmail(contract: PendingContract, companyName: string): Promise<boolean> {
  if (!env.RESEND_API_KEY) return false;

  const link = `${env.CORS_ORIGIN}/sign/${contract.uniqueLink}`;
...
<p style="color:#d1d5db;font-size:11px;text-align:center;margin-top:20px;">${companyName} — Rappel automatique</p>
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: env.RESEND_FROM || 'Orlode AI <noreply@music.zinakonect.com>',
    ...
    logger.error('[Reminder] Email failed', { error: err, email: contract.signatoryEmail });
    return false;
  }
}

async function processReminders(): Promise<void> {
  logger.info('[Reminder] Checking pending contracts...');
  ...
  try {
    ...
    for (const companyDoc of companiesSnap.docs) {
      ...
      for (const contractDoc of contractsSnap.docs) {
        ...
        const daysSinceLastReminder = lastReminderAt ? (Date.now() - lastReminderAt.getTime()) / 86_400_000 : Infinity;
        ...
        // Max 2 reminders
        if (remindersSent >= 2) shouldRemind = false;
        ...
        if (shouldRemind) {
          ...
          if (sent) {
            ...
            totalSent++;
            logger.info('[Reminder] Sent', { contractId: contract.id, email: contract.signatoryEmail, reminderNumber: remindersSent + 1 });
          }
        }
      }
    }

    logger.info('[Reminder] Done', { totalChecked, totalSent });
  } catch (err) {
    logger.error('[Reminder] Error during processing', { error: err });
  }
}

let reminderInterval: ReturnType<typeof setInterval> | null = null;

export function startContractReminders(): void {
  if (reminderInterval) return;

  // Run immediately on start
  setTimeout(() => processReminders(), 30_000); // 30s after server start

  // Then every 6 hours
  reminderInterval = setInterval(processReminders, CHECK_INTERVAL_MS);
  logger.info(`[Reminder] Started — checking every ${CHECK_INTERVAL_MS / 3_600_000}h`);
}

export function stopContractReminders(): void {
  if (reminderInterval) {
...
export { processReminders as runContractReminders };
