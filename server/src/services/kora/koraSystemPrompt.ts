 ...
 */
import { KoraProfile, KoraFact, Mood, topFactsForPrompt, getRecentMood, lastSessionSummary } from './koraMemoryService';
...
function timeOfDayMood(localHour: number): string {
  if (localHour < 5)  return 'milieu de nuit, ton très posé, peu de mots';
  if (localHour < 11) return 'matin, énergique mais pas mielleux';
  if (localHour < 14) return 'midi, neutre et pragmatique';
  if (localHour < 18) return 'après-midi, productif et concentré';
  if (localHour < 22) return 'soir, ton détendu, on prend le temps';
  return 'tard, voix douce, on ne rallonge pas si pas nécessaire';
}

function localHourFor(tz: string, now: Date): number {
  try {
...
function moodGuidance(mood: Mood | null): string {
  switch (mood) {
    case 'anxious':
    case 'angry':
...
function formatFactsBlock(facts: KoraFact[], language: string): string {
  if (facts.length === 0) {
    return language === 'fr'
...
function personalityBlock(personality: KoraProfile['personality'], language: string): string {
  const base = {
    fr: {
...
export async function buildKoraSystemPrompt(opts: BuildOpts): Promise<string> {
  const { profile } = opts;
  ...
  const [facts, mood, lastSession] = await Promise.all([
    topFactsForPrompt(profile.uid, profile.companyId, 20),
    getRecentMood(profile.uid, profile.companyId, 5),
    lastSessionSummary(profile.uid, profile.companyId),
  ]);
