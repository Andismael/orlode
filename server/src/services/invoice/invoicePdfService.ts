...
export interface InvoiceCompany {
  ...
  taxId?: string;
  legalForm?: string;
  /** IBAN for payment instructions */
  iban?: string;
  /** SWIFT/BIC code */
  swift?: string;
  /** Bank name */
  bankName?: string;
  /** Mobile money number for African business */
  mobileMoney?: string;
}

export interface InvoiceOrderData {
  id: string;
  /** Human-readable number — FAC-2026-0008, DEV-2026-0004, etc. Falls back to id slice if missing. */
  number?: string;
  clientName: string;
  clientEmail?: string;
  clientPhone?: string;
  /** Optional client address — adds professionalism */
  clientAddress?: string;
  items: Array<{ name: string; quantity: number; unitPrice: number }>;
  /** Subtotal HT (before tax). If not provided, computed from items. */
  totalHT?: number;
  /** Tax rate %. 0 if exempt. Default 0 in Africa, 18 in Côte d'Ivoire OHADA */
  taxRate?: number;
  /** Tax amount. Computed if not provided. */
  taxAmount?: number;
  /** Subtotal kept for backwards compat. If totalHT/totalTTC not given, this is treated as TTC. */
  subtotal: number;
  /** Total TTC (after tax). Computed if not provided. */
  totalTTC?: number;
  currency: string;
  status: string;
  paymentMethod?: string | null;
  paidAt?: { seconds?: number } | Date | string | null;
  createdAt?: { seconds?: number } | Date | string | null;
  dueDate?: { seconds?: number } | Date | string | null;
  /** Document type label: 'FACTURE' (default), 'DEVIS', 'AVOIR', etc. Drives header + footer text. */
  docType?: 'invoice' | 'quote' | 'credit_note';
  /** Quote validity date (only used when docType === 'quote'). */
  validUntil?: string;
  /** Free-form notes / message du client (footer) */
  notes?: string;
...
export async function loadCompanyForInvoice(companyId: string): Promise<InvoiceCompany> {
  try {
...
export async function loadOrderForInvoice(companyId: string, orderId: string): Promise<InvoiceOrderData | null> {
  try {
    const doc = await getFirestore().collection(`companies/${companyId}/orders`).doc(orderId).get();
    ...
    return {
      ...
      items:        (d['items'] as Array<{ name: string; quantity: number; unitPrice: number }>) ?? [],
      subtotal:     Number(d['subtotal'] ?? 0),
      currency:     (d['currency'] as string) ?? 'XOF',
      status:       (d['status'] as string) ?? 'pending_payment',
      paymentMethod: (d['paymentMethod'] as string) ?? null,
      paidAt:       (d['paidAt']     as { seconds?: number } | null) ?? null,
      createdAt:    (d['createdAt']  as { seconds?: number } | null) ?? null,
    };
  } catch { return null; }
}
...
export async function renderInvoicePdf(company: InvoiceCompany, order: InvoiceOrderData): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (logoBuf) {
        try { doc.image(logoBuf, 40, 18, { width: 44, height: 44 }); }
        catch { /* fallback */ }
      } else {
      ...
      if (clientLine) doc.text(clientLine, 52, clientBoxY + 36);
      if (order.clientAddress) doc.text(order.clientAddress, 320, clientBoxY + 20, { width: 230 });
      ...
      for (const item of order.items) {
        ...
        if (zebra % 2 === 1) doc.rect(40, rowY, 515, 20).fill('#f9fafb');
      ...
      if (!isQuote && (company.iban || company.mobileMoney || company.bankName)) {
        ...
        let payY = totY + 22;
        if (company.bankName) { doc.text(`Banque : ${company.bankName}`, 52, payY, { width: 230 }); payY += 11; }
        if (company.iban) { doc.text(`IBAN : ${company.iban}`, 52, payY, { width: 230 }); payY += 11; }
        if (company.swift) { doc.text(`SWIFT/BIC : ${company.swift}`, 52, payY, { width: 230 }); payY += 11; }
        if (company.mobileMoney) { doc.text(`Mobile Money : ${company.mobileMoney}`, 52, payY, { width: 230 }); }
      }
