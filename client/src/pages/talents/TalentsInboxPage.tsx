/**
 * Orlode Talents — recruiter inbox at /talents/inbox.
 *
 * Recruiters see all the conversations they've initiated with candidates.
 * Candidate replies (arriving via WhatsApp on the platform number) are
 ...
 * candidate's number (relayed through the Orlode platform).
 *
...
interface ConvSummary {
  ...
  lastMessageFrom: 'recruiter' | 'candidate';
...
export default function TalentsInboxPage() {
  useSEO({
    title: 'Inbox Talents — Orlode',
    description: 'Conversations avec les candidats que tu as contactés sur Orlode Talents.',
    path: '/talents/inbox',
  });
  ...
  const loadConvs = async () => {
    try {
      const r: any = await api.get('/talents/conversations');
  ...
  useEffect(() => {
    ...
    setMessages(null);
    api.get(`/talents/conversations/${active}/messages`)
    ...
    // Mark read
    api.post(`/talents/conversations/${active}/mark-read`, {}).catch(() => {});
  ...
  const send = async () => {
    ...
    setError(null);
    try {
      await api.post(`/talents/conversations/${active}/reply`, { message: draft.trim() });
      setDraft('');
      // Optimistic: refresh thread
      const r: any = await api.get(`/talents/conversations/${active}/messages`);
  ...
  return (
    <div className="m-root" style={{ minHeight: '100vh', background: M.cream, display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{
        ...
        <div style={{ flex: 1 }}>
          <div className="m-display" style={{ fontSize: 15, fontWeight: 700 }}>Inbox Talents</div>
          ...
          {convs?.length === 0 && (
            <div style={{
              ...
              <span style={{ fontSize: 11 }}>
                Va sur le <Link to="/talents/feed" style={{ color: M.emeraldDeep }}>feed</Link> et contacte un talent pour démarrer.
              </span>
            </div>
          )}
          {convs?.map(c => {
            ...
            return (
              <button key={c.id} onClick={() => setActive(c.id)} className="tap-card" style={{
                ...
                background: isActive ? M.emeraldSoft : 'transparent',
                border: 'none',
                ...
                <div style={{
                  ...
                  {c.talentDisplayName?.[0]?.toUpperCase() ?? '?'}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 6 }}>
                    ...
                    }}>{c.talentDisplayName || 'Candidat'}</span>
                  ...
                  <div style={{
                    ...
                    {c.lastMessageFrom === 'candidate' ? '↪ ' : ''}{c.lastMessageText}
                  </div>
                </div>
                {c.unreadByRecruiter > 0 && (
                  <span className="m-pill" style={{
                    background: M.coralDeep, color: M.cream,
                    fontSize: 9, padding: '2px 6px',
                    flexShrink: 0, marginTop: 4,
                  }}>{c.unreadByRecruiter}</span>
                )}
              </button>
            );
          })}
        ...
        {active && (
          <section style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#E5DDD5' }}>
            ...
            <div className="md:hidden" style={{
              padding: '8px 14px', background: M.greenDeep, color: M.cream,
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 700 }}>
                  {activeConv?.talentDisplayName ?? 'Candidat'}
                </div>
                <div style={{ fontSize: 10, opacity: 0.8 }}>via WhatsApp · Orlode Talents</div>
              </div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px 12px 6px' }}>
              {messages === null ? (
                <div style={{ textAlign: 'center', padding: 40 }}>
                  <Loader2 className="animate-spin" size={20} color={M.greenDeep} />
                </div>
              ) : (
                messages.map(m => {
                  const isMe = m.from === 'recruiter';
                  return (
                    <div key={m.id} style={{
                      display: 'flex',
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <textarea
                  ...
                  placeholder="Réponds au candidat…"
                  rows={1}
                  style={{
                ...
                <button onClick={() => void send()} disabled={sending || !draft.trim()} className="tap-card" style={{
                  ...
                  boxShadow: `0 6px 14px -4px ${M.emerald}`,
                }}>
