...
function getTiktokAuthUrl(state, redirectUri) {
...
async function exchangeTiktokCode(code, redirectUri) {
    ...
    if (!clientKey || !clientSecret)
        throw new Error('TIKTOK_CLIENT_KEY / SECRET not configured');
...
async function refreshTiktokToken(refreshToken) {
    ...
    const clientSecret = process.env['TIKTOK_CLIENT_SECRET'] ?? '';
    if (!clientKey || !clientSecret)
        throw new Error('TIKTOK creds not configured');
    const body = new URLSearchParams({
        client_key: clientKey,
        client_secret: clientSecret,
    ...
    if (!r.ok || !j.access_token)
        throw new Error(j.error ?? `Refresh failed (HTTP ${r.status})`);
    return {
 ...
 */
async function uploadVideoToTiktok(args) {
    ...
    logger_1.logger.info('[TikTok] upload complete', { publishId, status, directPublish: !!directPublish });
    return { publishId, status };
}
//# sourceMappingURL=tiktokPublisher.js.map