...
exports.tryHandleKoraWhatsApp = tryHandleKoraWhatsApp;
/**
 ...
 */
function customNameTriggers(name) {
    const n = (name || '').trim();
    if (!n || n.toLowerCase() === 'kora')
        return { oneShot: null, session: null };
 ...
 */
async function resolveUidByPhone(companyId, fromPhoneE164) {
    if (!fromPhoneE164)
        return null;
    ...
    try {
        ...
        const hit = fallback.docs.find(d => String(d.data().phoneE164 ?? '').replace(/\D/g, '') === norm);
        return hit?.id ?? null;
    }
    catch (err) {
        logger_1.logger.warn('[Kora] resolveUidByPhone failed', { companyId, error: String(err) });
        return null;
    }
}
async function readSession(companyId, fromKey) {
    try {
        const snap = await (0, firebase_config_1.getFirestore)().doc(`companies/${companyId}/koraWaSessions/${fromKey}`).get();
...
function classifyOwnerMessage(message) {
    const msg = message.trim();
    // 1. Admin / sensitive — never Kora.
    if (/^\s*@admin\b/i.test(msg))
        return 'admin';
    if (/^\s*\d{4,6}\s*$/.test(msg))
        return 'admin'; // bare OTP code
    if (/\b(otp|code\s+(de\s+)?confirmation|valider\s+la\s+commande|confirme\s+le\s+paiement|annuler\s+la\s+commande)\b/i.test(msg))
        return 'admin';
    // 2. Business — commerce/customer-flow language.
    if (/\b(prix|tarif|disponible|en\s+stock|livraison|commande|facture|r[ée]server|catalogue|offre|devis|paiement|wave|orange\s*money|cash|combien\s+coûte|c'est\s+combien|menu|chambre)\b/i.test(msg))
        return 'business';
    // 3. Kora — personal markers.
    if (/\b(rappelle[- ]moi|note\s+que|j'ai\s+oubli[ée]|j'ai\s+un\s+rdv|demain\s+je|hier\s+je|je\s+(suis|me\s+sens)\s+(fatigu[ée]|content|stress[eé]?|[ée]puis[eé]|seul|triste|heureux)|je\s+galère|aide[- ]moi\s+à\s+organiser|tu\s+te\s+souviens|comment\s+je\s+vais|qu'est[- ]ce\s+que\s+tu\s+sais\s+de\s+moi|j'ai\s+besoin\s+de\s+(parler|réfléchir)|note\s+ça|enregistre\s+ça)\b/i.test(msg))
        return 'kora';
...
async function writeSession(companyId, fromKey, uid, active) {
    await (0, firebase_config_1.getFirestore)().doc(`companies/${companyId}/koraWaSessions/${fromKey}`).set({ uid, active, lastMessageAt: firestore_1.FieldValue.serverTimestamp() }, { merge: true });
}
/**
 ...
 * `handled` is true.
 */
async function tryHandleKoraWhatsApp(params) {
    ...
    if ((!isOneShot && !isSession) && uid) {
        try {
            const profilePeek = await (0, koraMemoryService_1.getOrCreateProfile)({ uid, companyId });
            const { oneShot, session: sessionRe } = customNameTriggers(profilePeek.assistantName);
            if (oneShot && oneShot.test(message))
                isOneShot = true;
            if (!isOneShot && sessionRe && sessionRe.test(message))
                isSession = true;
        }
        catch { /* fall through */ }
    }
    ...
    const profile = await (0, koraMemoryService_1.getOrCreateProfile)({ uid, companyId, defaults: { phoneE164: fromPhoneE164 } });
    if (!profile.enabled)
        return { handled: false };
    ...
    if (!profile.phoneE164) {
        await (0, koraMemoryService_1.updateProfile)(uid, companyId, { phoneE164: fromPhoneE164 });
    }
    ...
    if (isOneShot) {
        ...
        if (customOneShot)
            cleaned = cleaned.replace(customOneShot, '').trim();
    }
    else if (isSession) {
        cleaned = cleaned.replace(SESSION_RE, '').trim();
        if (customSession)
            cleaned = cleaned.replace(customSession, '').trim();
    }
    const userMessage = cleaned || `Salut ${profile.assistantName || 'Kora'}.`;
    const result = await (0, koraChatService_1.koraReply)({ uid, companyId, userMessage });
...
//# sourceMappingURL=koraWhatsAppHandler.js.map