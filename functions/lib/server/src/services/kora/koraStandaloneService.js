...
exports.TRIAL_DAYS = exports.STANDALONE_COMPANY_ID = void 0;
exports.processStandaloneInbound = processStandaloneInbound;
exports.markStandaloneActive = markStandaloneActive;
exports.migrateStandaloneToWorkspace = migrateStandaloneToWorkspace;
/**
...
const koraChatService_1 = require("./koraChatService");
const logger_1 = require("../../utils/logger");
exports.STANDALONE_COMPANY_ID = '_standalone';
exports.TRIAL_DAYS = 7;
/** Normalise a phone number to a Firestore-safe key. */
function phoneKey(phoneE164) {
    return phoneE164.replace(/[^0-9]/g, '');
}
/** Path to the standalone state document for this phone. */
function stateDocPath(phoneE164) {
    return `companies/${exports.STANDALONE_COMPANY_ID}/koraStandaloneState/${phoneKey(phoneE164)}`;
}
async function readStandaloneState(phoneE164) {
    try {
        const snap = await (0, firebase_config_1.getFirestore)().doc(stateDocPath(phoneE164)).get();
        if (!snap.exists)
            return null;
        ...
        logger_1.logger.warn('[KoraStandalone] readState failed', { error: String(err) });
        return null;
    }
}
async function writeStandaloneState(phoneE164, patch) {
    await (0, firebase_config_1.getFirestore)().doc(stateDocPath(phoneE164)).set({ ...patch, updatedAt: firestore_1.FieldValue.serverTimestamp() }, { merge: true });
}
/** Days remaining in the free trial (negative if expired). */
function trialDaysRemaining(state) {
    if (!state.trialStartedAt)
        return exports.TRIAL_DAYS;
    const elapsedDays = (Date.now() - state.trialStartedAt.getTime()) / 86400000;
    return exports.TRIAL_DAYS - Math.floor(elapsedDays);
}
/**
 * Top-level: handle one inbound WhatsApp message for a standalone user.
 * Returns the text to reply with.
 */
async function processStandaloneInbound(params) {
    ...
    if (state.subscriptionStatus === 'trial_expired' || (state.subscriptionStatus === 'trialing' && days <= 0)) {
        ...
        return {
            reply: "Ton essai de 7 jours est terminé. Pour continuer (et garder tout ce que je sais de toi) : https://orlode.com/kora/upgrade?phone=" +
    ...
    const reply = await (0, koraChatService_1.koraReply)({
        uid,
        companyId: exports.STANDALONE_COMPANY_ID,
        userMessage: message,
    });
 ...
 */
async function advanceOnboarding(phoneE164, uid, state, message) {
    const trimmed = message.trim();
    switch (state.onboardingStep) {
        case 'ask_firstname': {
            const firstName = trimmed.slice(0, 40);
            if (!firstName)
                return { reply: 'Juste ton prénom suffit.' };
            await (0, koraMemoryService_1.updateProfile)(uid, exports.STANDALONE_COMPANY_ID, { firstName });
        ...
        case 'ask_assistant_name': {
            ...
            await (0, koraMemoryService_1.updateProfile)(uid, exports.STANDALONE_COMPANY_ID, { assistantName: name });
        ...
        case 'ask_language': {
            ...
            const language = t.startsWith('EN') && !t.startsWith('FR') ? 'en' : 'fr';
            await (0, koraMemoryService_1.updateProfile)(uid, exports.STANDALONE_COMPANY_ID, { language });
            await writeStandaloneState(phoneE164, {
                onboardingStep: 'ready',
                trialStartedAt: new Date(),
            });
            // Re-read the profile to fetch the name we just set.
            const profile = await (0, koraMemoryService_1.getOrCreateProfile)({ uid, companyId: exports.STANDALONE_COMPANY_ID });
            const name = profile.assistantName || 'Kora';
            return {
                reply: `C'est bon, on est prêts.\n\nTu as ${exports.TRIAL_DAYS} jours d'essai. Parle-moi de ce qui se passe dans ta vie, de tes projets, de ce qui te trotte dans la tête. Plus tu m'en dis, plus je deviens vraiment ton ${name}.\n\nQuelques exemples pour commencer :\n• "Rappelle-moi demain 9h d'appeler mon banquier"\n• "Je galère sur un dossier client, je sais pas comment relancer"\n• "Je suis content, j'ai signé un nouveau contrat"\n\nÀ toi.`,
            };
        }
        default:
 ...
 */
async function markStandaloneActive(phoneE164, stripeSubscriptionId) {
    await writeStandaloneState(phoneE164, {
 ...
 */
async function migrateStandaloneToWorkspace(params) {
    const { phoneE164, targetCompanyId, targetUid } = params;
    const srcUid = phoneKey(phoneE164);
    const db = (0, firebase_config_1.getFirestore)();
    const srcRoot = `companies/${exports.STANDALONE_COMPANY_ID}/koraProfiles/${srcUid}`;
    ...
    if (srcProfile.exists) {
        await db.doc(dstRoot).set({ ...srcProfile.data(), uid: targetUid, companyId: targetCompanyId, updatedAt: firestore_1.FieldValue.serverTimestamp() }, { merge: true });
    }
    const subs = ['facts', 'sessions', 'messages', 'reminders', 'moodLog', 'events'];
    const counts = {};
...
//# sourceMappingURL=koraStandaloneService.js.map