 ...
 */
async function renderContractPdf(company, employee) {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            if (logoBuf) {
                try {
                    doc.image(logoBuf, 40, 22, { width: 46, height: 46 });
                }
                catch { /* fallback */ }
            }
            else {
            ...
            if (company.legalForm) {
                doc.text(company.legalForm, 100, line);
                line += 11;
            }
            if (company.address) {
                doc.text(company.address, 100, line);
                line += 11;
            }
            const contact = [company.phone, company.email].filter(Boolean).join('  ·  ');
            if (contact)
                doc.text(contact, 100, line);
            ...
            if (company.legalForm)
                doc.text(company.legalForm, 50, doc.y);
            if (company.address)
                doc.text(company.address, 50, doc.y);
            if (company.taxId)
                doc.text(`N° ${company.taxId}`, 50, doc.y);
            ...
            doc.font('Helvetica').fontSize(10).fillColor('#374151');
            if (employee.email)
                doc.text(employee.email, 50, doc.y);
            if (employee.phone)
                doc.text(employee.phone, 50, doc.y);
...
//# sourceMappingURL=contractPdfService.js.map