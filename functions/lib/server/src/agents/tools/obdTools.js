...
exports.OBD_TOOL_NAMES = exports.OBD_TOOLS = exports.vehicleHistoryTool = exports.decodeOBDTool = void 0;
/**
...
// ── OBD Decode Tool ─────────────────────────────────────────────────────────
exports.decodeOBDTool = genkit_config_1.ai.defineTool({
    name: 'decodeOBD',
    ...
    inputSchema: zod_1.z.object({
        codes: zod_1.z.array(zod_1.z.string()).describe('OBD-II codes to decode (e.g. ["P0300", "P0420"])'),
    ...
    outputSchema: zod_1.z.object({
        results: zod_1.z.array(zod_1.z.object({
            ...
            estimatedCost: zod_1.z.string(),
            urgency: zod_1.z.string(),
        })),
        overallSeverity: zod_1.z.string(),
        totalEstimatedCost: zod_1.z.string(),
    ...
    const results = input.codes.map(code => {
        ...
        const categories = { P: 'Powertrain', B: 'Body', C: 'Chassis', U: 'Network' };
        return {
            code: upper,
    ...
    for (const code of input.codes) {
        ...
        if (known) {
            minTotal += known.estimatedCostUSD[0];
            maxTotal += known.estimatedCostUSD[1];
        }
    }
    const recommendations = [];
    if (severities.includes('critical'))
        recommendations.push('ARRET IMMEDIAT recommande — risque d\'endommagement grave');
    if (severities.includes('high'))
        recommendations.push('Reparer dans les plus brefs delais');
    if (results.some(r => r.category === 'Transmission'))
        recommendations.push('Verifier le niveau et la couleur de l\'huile de boite');
    if (results.some(r => r.category === 'Ignition'))
        recommendations.push('Verifier bougies, bobines et compression');
    if (results.some(r => r.category === 'Emissions'))
        recommendations.push('Prevoir un controle technique apres reparation');
    if (results.some(r => r.category === 'Cooling'))
        recommendations.push('Verifier le niveau de liquide de refroidissement immediatement');
    if (input.vehicleInfo?.mileage && input.vehicleInfo.mileage > 150000)
        recommendations.push('Kilometrage eleve — prevoir un bilan complet');
    ...
    try {
        ...
        await db.collection(`companies/${input.companyId}/diagnostics`).doc((0, helpers_1.generateId)()).set({
...
exports.vehicleHistoryTool = genkit_config_1.ai.defineTool({
    name: 'vehicleHistory',
    description: 'Get repair history for a vehicle (by license plate or client name). Shows past diagnostics, repairs, and costs.',
    inputSchema: zod_1.z.object({
        companyId: zod_1.z.string(),
        search: zod_1.z.string().describe('License plate, VIN, or client name'),
    }),
...
exports.OBD_TOOLS = [exports.decodeOBDTool, exports.vehicleHistoryTool];
exports.OBD_TOOL_NAMES = ['decodeOBD', 'vehicleHistory'];
//# sourceMappingURL=obdTools.js.map