...
exports.securityScoreTool = genkit_config_1.ai.defineTool({
    ...
    try {
        ...
        logger_1.logger.error('[sec_getSecurityScore] Error:', err);
...
exports.incidentResponseTool = genkit_config_1.ai.defineTool({
    ...
    try {
        ...
        logger_1.logger.error('[sec_reportIncident] Error:', err);
...
exports.accessReviewTool = genkit_config_1.ai.defineTool({
    ...
    try {
        ...
        logger_1.logger.error('[sec_reviewAccess] Error:', err);
...
exports.complianceCheckerTool = genkit_config_1.ai.defineTool({
    ...
    try {
        ...
        logger_1.logger.error('[sec_checkCompliance] Error:', err);
...
exports.cybersecurityAgentFlow = genkit_config_1.ai.defineFlow({ name: 'cybersecurityAgent', inputSchema: INPUT, outputSchema: OUTPUT }, async ({ request, companyId, userId, language }) => {
    try {
        logger_1.logger.info(`[CybersecurityAgent] Request: "${request.slice(0, 80)}"`);
        ...
        let response = await genkit_config_1.ai.generate({
            model: genkit_config_1.GEMINI_FLASH,
            system: `You are the Cybersecurity Agent PRO (virtual CISO) for a company.
CompanyID: ${companyId}. UserID: ${userId ?? 'unknown'}.
You monitor threats, manage incidents with full workflow, scan vulnerabilities, run phishing simulations,
enforce compliance (RGPD/ISO27001/SOC2/PCI-DSS/NIST/HIPAA), manage security policies, and audit access.

CAPABILITIES:
- Security score with multi-category breakdown
- Incident response with timeline, SLA, escalation
- Vulnerability scanning with CVE tracking
- Phishing simulation campaigns
- Compliance automation with controls
- Security policy management
- Threat intelligence feed (SIEM-lite)
- AI-powered security audits

PRIORITY: Security comes first. Report P1/P2 incidents to human admins immediately.
NEVER downplay a security alert. Log everything. Recommend least-privilege principle.
${langInstr}`,
            prompt: request, tools: ALL_TOOLS, config: { temperature: 0.2 },
        });
        let loopCount = 0;
        ...
        return { response: 'Erreur dans l\'agent cybersecurite.', alertLevel: 'none', requiresHumanEscalation: false };
    }
});
