...
exports.createInvoiceTool = genkit_config_1.ai.defineTool({
    ...
    outputSchema: zod_1.z.object({ invoiceId: zod_1.z.string(), number: zod_1.z.string(), totalHT: zod_1.z.number(), totalTTC: zod_1.z.number(), message: zod_1.z.string() }),
...
exports.submitExpenseTool = genkit_config_1.ai.defineTool({
    ...
    outputSchema: zod_1.z.object({ expenseId: zod_1.z.string(), message: zod_1.z.string() }),
...
exports.getCashFlowTool = genkit_config_1.ai.defineTool({
    ...
    outputSchema: zod_1.z.object({ period: zod_1.z.string(), inflows: zod_1.z.number(), outflows: zod_1.z.number(), balance: zod_1.z.number(), currency: zod_1.z.string(), alerts: zod_1.z.array(zod_1.z.string()) }),
    ...
    if (pendingExpenses.data().count > 0)
        alerts.push(`${pendingExpenses.data().count} note(s) de frais en attente.`);
...
exports.convertCurrencyTool = genkit_config_1.ai.defineTool({
    ...
    outputSchema: zod_1.z.object({ original: zod_1.z.number(), converted: zod_1.z.number(), from: zod_1.z.string(), to: zod_1.z.string(), rate: zod_1.z.number() }),
...
exports.createMultiCurrencyInvoiceTool = genkit_config_1.ai.defineTool({
    ...
    outputSchema: zod_1.z.object({ invoiceId: zod_1.z.string(), number: zod_1.z.string(), totalTTC: zod_1.z.number(), currency: zod_1.z.string(), eurEquivalent: zod_1.z.number(), message: zod_1.z.string() }),
}, async ({ companyId, client, clientEmail, items, currency, dueInDays, taxRate, notes }) => {
    const result = await (0, exports.createInvoiceTool)({ companyId, client, clientEmail, items, currency, dueInDays, taxRate, notes });
...
exports.exportFinanceTool = genkit_config_1.ai.defineTool({
    ...
    outputSchema: zod_1.z.object({ csv: zod_1.z.string(), rowCount: zod_1.z.number(), message: zod_1.z.string() }),
...
exports.createRecurringInvoiceTool = genkit_config_1.ai.defineTool({
    ...
    outputSchema: zod_1.z.object({ recurringId: zod_1.z.string(), frequency: zod_1.z.string(), nextInvoiceDate: zod_1.z.string(), message: zod_1.z.string() }),
...
exports.getRecurringInvoicesTool = genkit_config_1.ai.defineTool({
    ...
    inputSchema: zod_1.z.object({ companyId: zod_1.z.string() }),
    outputSchema: zod_1.z.object({ recurring: zod_1.z.array(zod_1.z.object({ id: zod_1.z.string(), client: zod_1.z.string(), frequency: zod_1.z.string(), totalTTC: zod_1.z.number(), nextInvoiceDate: zod_1.z.string(), status: zod_1.z.string(), generatedCount: zod_1.z.number() })) }),
}, async ({ companyId }) => {
    const snap = await (0, firebase_config_1.getFirestore)().collection(`companies/${companyId}/recurringInvoices`).limit(50).get();
    return { recurring: snap.docs.map(d => { const data = d.data(); return { id: d.id, client: data['client'] ?? '', frequency: data['frequency'] ?? '', totalTTC: data['totalTTC'] ?? 0, nextInvoiceDate: data['nextInvoiceDate'] ?? '', status: data['status'] ?? 'active', generatedCount: data['generatedCount'] ?? 0 }; }) };
});
// ══════════════════════════════════════════════════════════════════════════════
