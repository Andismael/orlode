...
exports.validateFirebase = validateFirebase;
exports.saveBYOEConfig = saveBYOEConfig;
...
async function setupChat(req, res) {
    ...
    logger_1.logger.info(`[SetupController] Agent chat: step=${step} company=${companyId}`);
    const result = await (0, setupAgent_1.setupAgentFlow)({ companyId, message, step, context });
...
async function testApiKey(req, res) {
    ...
    if (!provider || !apiKey)
        throw new error_middleware_1.AppError('provider and apiKey are required', 400);
    if (!['gemini', 'claude', 'openai'].includes(provider)) {
        throw new error_middleware_1.AppError('provider must be gemini, claude, or openai', 400);
    }
    try {
        switch (provider) {
            case 'gemini': {
...
async function validateFirebase(req, res) {
    const { projectId: rawProjectId, serviceAccountKey } = req.body;
    const projectId = rawProjectId?.trim();
    if (!projectId || !serviceAccountKey) {
        throw new error_middleware_1.AppError('projectId and serviceAccountKey are required', 400);
    }
...
async function saveBYOEConfig(req, res) {
    const companyId = req.user?.companyId;
    if (!companyId)
        throw new error_middleware_1.AppError('Company ID required', 400);
    const { firebaseProjectId, serviceAccountKey, storageBucket, region, geminiApiKey, claudeApiKey, openaiApiKey, } = req.body;
    if (!firebaseProjectId || !serviceAccountKey || !geminiApiKey) {
        throw new error_middleware_1.AppError('firebaseProjectId, serviceAccountKey, and geminiApiKey are required', 400);
    }
