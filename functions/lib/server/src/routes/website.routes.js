...
router.post('/leads/:companyId', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { companyId } = req.params;
    if (!companyId)
        throw new error_middleware_1.AppError('companyId required', 400);
    const { name, email, phone, message, source } = req.body;
    if (!email && !phone)
        throw new error_middleware_1.AppError('email or phone required', 400);
    ...
    if (!cfg.exists)
        throw new error_middleware_1.AppError('Site not found', 404);
    ...
    logger_1.logger.info('[Website] Lead captured', { companyId, leadId: ref.id });
...
router.get('/preview/:companyId', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { companyId } = req.params;
    const db = (0, firebase_config_1.getFirestore)();
    const doc = await db.collection(`companies/${companyId}/website`).doc('config').get();
    if (!doc.exists)
        throw new error_middleware_1.AppError('Site not found — generate it first via /admin/website/builder.', 404);
    const data = doc.data();
...
<p style="color:#6b7280;font-size:14px;">L'administrateur peut publier le site depuis le builder pour le rendre visible publiquement.</p>
    ...
    const importedHtml = content['rawHtml'] ?? data['rawHtml'] ?? '';
    let html;
    if (importedHtml && importedHtml.trim().length > 100) {
        const widgetEnabled = data['widgetEnabled'] ?? true;
        const color = data['color'] ?? data['primaryColor'] ?? '#6c3ce0';
        ...
        const { generateStaticHTML } = await Promise.resolve().then(() => __importStar(require('../agents/website.agent')));
        html = generateStaticHTML(content, data['companyName'] ?? data['name'] ?? 'Mon Site', data['color'] ?? data['primaryColor'] ?? '#6c3ce0', companyId, data['widgetEnabled'] ?? true, data['template'] ?? undefined);
    }
    res.set('Content-Type', 'text/html; charset=utf-8');
...
router.put('/config', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (update['status'] === undefined)
        update['status'] = 'draft';
    if (update['widgetEnabled'] === undefined)
...
router.post('/import', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const companyId = req.user?.companyId;
    if (!companyId)
        throw new error_middleware_1.AppError('Company required', 400);
    const { rawHtml, importedFrom, companyName, color, publish } = req.body;
    if (!rawHtml?.trim())
        throw new error_middleware_1.AppError('rawHtml required', 400);
    if (rawHtml.length > 5000000)
        throw new error_middleware_1.AppError('HTML too large (max 5MB)', 413);
    ...
    if (!looksLikeHtml)
        throw new error_middleware_1.AppError('Content does not appear to be HTML', 400);
    const db = (0, firebase_config_1.getFirestore)();
    ...
    logger_1.logger.info('[Website] Imported HTML', { companyId, source: importedFrom, sizeBytes: rawHtml.length });
    res.json({
        success: true,
        data: {
...
router.post('/generate', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!companyId)
        throw new error_middleware_1.AppError('Company required', 400);
    const { template = 'vitrine', style = 'modern', color = '#6c3ce0', pages = ['home', 'about', 'services', 'contact'], language = 'fr', provider, // 'claude' | 'gemini' | 'openai' — recorded but real switch is a follow-up
     } = req.body;
    const { generateWebsiteTool } = await Promise.resolve().then(() => __importStar(require('../agents/website.agent')));
    try {
        ...
        if (provider) {
            const db = (0, firebase_config_1.getFirestore)();
            await db.collection(`companies/${companyId}/website`).doc('config').set({ generatedByProvider: provider, generatedByLabel: provider, updatedAt: firestore_1.FieldValue.serverTimestamp() }, { merge: true });
        }
        res.json({ success: true, data: out });
    }
    catch (err) {
        logger_1.logger.error('[Website] Generation failed', { err: String(err), companyId });
        throw new error_middleware_1.AppError(`Generation failed: ${err.message ?? String(err)}`, 502);
    }
}));
// POST /api/website/update — AI modifications (chat from the manual builder)
router.post('/update', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const companyId = req.user?.companyId;
    if (!companyId)
        throw new error_middleware_1.AppError('Company required', 400);
    const { instruction } = req.body;
    if (!instruction?.trim())
        throw new error_middleware_1.AppError('instruction required', 400);
    const { updateWebsiteTool } = await Promise.resolve().then(() => __importStar(require('../agents/website.agent')));
    try {
        const out = await updateWebsiteTool({ companyId, instruction });
        res.json({ success: true, data: out });
    }
    catch (err) {
        logger_1.logger.error('[Website] Update failed', { err: String(err), companyId });
        throw new error_middleware_1.AppError(`Update failed: ${err.message ?? String(err)}`, 502);
    }
}));
// GET /api/website/leads — list captured leads
