...
function handoffPageFor(languageCode) {
    const lc = (languageCode || '').toLowerCase();
    if (lc.startsWith('fr'))
        return process.env.DIALOGFLOW_HANDOFF_PAGE_FR || process.env.DIALOGFLOW_HANDOFF_PAGE || '';
    if (lc.startsWith('en'))
        return process.env.DIALOGFLOW_HANDOFF_PAGE_EN || process.env.DIALOGFLOW_HANDOFF_PAGE || '';
...
router.post('/webhook', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (expected) {
        ...
        const customHeader = req.headers['x-orlode-webhook-secret'] || '';
        let providedSecret = '';
        ...
        if (providedSecret !== expected) {
            logger_1.logger.warn('[CCAI] Webhook unauthorized — bad secret');
            return res.status(401).json({ error: 'unauthorized' });
        }
    }
    else {
        logger_1.logger.warn('[CCAI] DIALOGFLOW_WEBHOOK_SECRET not set — webhook is OPEN. Set it in env before production.');
    }
    try {
        const body = req.body || {};
        // ── 2. Parse Dialogflow CX request (camelCase per Google docs) ──────────
        const sessionPath = body.sessionInfo?.session || '';
        ...
        const userText = (body.text || body.transcript || '').toString();
        const intent = body.intentInfo?.displayName || 'unknown';
        ...
        logger_1.logger.info('[CCAI] Webhook received', { sessionId, intent, queryLen: queryText.length, lang: languageCode, isNoInput });
...
`;
        ...
        try {
            ...
            logger_1.logger.error('[CCAI] Genkit generation failed', genkitErr);
        }
        if (!text) {
            text = isFr
        ...
        if (shouldHandoff && handoff) {
            logger_1.logger.info('[CCAI] Handoff triggered', { sessionId, lang: languageCode, reason: explicitHandoff ? 'explicit' : lowConfidence ? 'low_confidence' : 'user_request' });
            return res.json({
                fulfillmentResponse: {
                    messages: [{
                            text: { text: [isFr
        ...
        logger_1.logger.error('[CCAI] Webhook error', error);
...
//# sourceMappingURL=ccai.routes.js.map