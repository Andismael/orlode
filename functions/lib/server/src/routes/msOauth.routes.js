...
function isPersonal(service) { return PERSONAL_SERVICES.has(service); }
function storagePath(service, companyId, uid) {
...
router.get('/start', auth_middleware_1.authMiddleware, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!SERVICE_SCOPES[service])
        throw new error_middleware_1.AppError(`Unknown service: ${service}`, 400);
    const creds = appCreds();
    if (!creds)
        throw new error_middleware_1.AppError('Microsoft OAuth non configuré (MS_CLIENT_ID manquant).', 503);
    ...
    const authUrl = `https://login.microsoftonline.com/${tenantAuthority()}/oauth2/v2.0/authorize` +
...
router.get('/callback', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const stateEnc = req.query['state'];
    const errorParam = req.query['error'];
    const errorDesc = req.query['error_description'];
    const base = process.env['CORS_ORIGIN'] ?? 'https://orlode.com';
    if (errorParam) {
        logger_1.logger.warn('[MS OAuth] User denied or error', { errorParam, errorDesc });
        return res.redirect(`${base}/connectors?ms=error&reason=${encodeURIComponent(errorDesc ?? errorParam)}`);
    }
    if (!code || !stateEnc)
        return res.redirect(`${base}/connectors?ms=error&reason=missing_code`);
    let state;
    try {
        state = JSON.parse(Buffer.from(stateEnc, 'base64url').toString('utf-8'));
    }
    catch {
        return res.redirect(`${base}/connectors?ms=error&reason=invalid_state`);
    }
    const creds = appCreds();
    if (!creds)
        return res.redirect(`${base}/connectors?ms=error&reason=app_not_configured`);
    try {
        ...
        if (!tokenRes.ok) {
            ...
            logger_1.logger.error('[MS OAuth] Token exchange failed', { status: tokenRes.status, errText: errText.slice(0, 300) });
        ...
        await db.collection(path).doc(state.service).set({
            ...
            accessTokenEncrypted: (0, encryption_1.encrypt)(tokens.access_token),
            refreshTokenEncrypted: tokens.refresh_token ? (0, encryption_1.encrypt)(tokens.refresh_token) : null,
        ...
        logger_1.logger.info('[MS OAuth] Connected', { companyId: state.companyId, service: state.service, user: profile?.userPrincipalName });
        res.redirect(`${base}/connectors?ms=success&service=${state.service}`);
    }
    catch (err) {
        logger_1.logger.error('[MS OAuth] Callback failed', { err: String(err) });
...
router.delete('/:service', auth_middleware_1.authMiddleware, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const companyId = req.user?.companyId;
    const uid = req.user?.uid;
    if (!companyId || !uid)
        throw new error_middleware_1.AppError('Auth required', 401);
...
//# sourceMappingURL=msOauth.routes.js.map